function RegressionOnSSCorticalRes(fsdir,subj,mepi_file,epi_file,TR,COIs,clus_file,motion_file,roisc_file,outdir)

n_temporal = 3;

aparc_file  = fullfile(fsdir,subj,'mri/aparc.a2009s+aseg.mgz');

% labels.name = {'hyppoL','hyppoR','AmygL','AmygR','ThalL','ThalR','PutaL','PutaR','CaudL','CaudR','CerebCL','CerebCR','CerebWL','CerebWR','PallidumL','PallidumR','VentralL','VentralR'};
% labels.ind  = [17 53 18 54 10 49 12 51 11 50 8 47 7 46 13 52 28 60];

labels.name = {'hyppoL','hyppoR','AmygL','AmygR','ThalL','ThalR','PutaL','PutaR','CaudL','CaudR','PallidumL','PallidumR','VentralL','VentralR'};
labels.ind  = [17 53 18 54 10 49 12 51 11 50 13 52 28 60];

%% Create subcortical ROIs
if filexist(roisc_file)
    cmd = sprintf('rm -f %s',roisc_file);
    unix(cmd);
end
CreateSCRoiBySubjectRes(aparc_file,mepi_file,roisc_file,labels);
