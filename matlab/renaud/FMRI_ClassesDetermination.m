...
st_write_nifti(tMaps,header,[datapath filesep 'Classes' filesep 'tMapsClass'])

unix(['mv ', datapath filesep 'rmaskB.nii ', datapath filesep 'Classes' filesep 'maskB.nii']);

V     = spm_vol([datapath filesep 'Classes' filesep 'maskB.nii']);
maskB = spm_read_vols(V);
maskB = maskB > 0;

resClust.compsName   = compsName;
resClust.maskB       = maskB;
resClust.hier        = hier;
resClust.contrib     = contrib;
resClust.timeCourses = timeCourses;
resClust.header_sica = header_sica;
resClust.header      = header;
