...
if ~exist(fullfile(sica.resfolder,'param_normalize.mat'))
    params_normalize = spm_normalise(VG,VF,matname,VWG,VWF,opt_normalize.estimate);
    save(fullfile(sica.resfolder,'param_normalize.mat'),'params_normalize');
else
    load(fullfile(sica.resfolder,'param_normalize.mat'),'params_normalize');
end
