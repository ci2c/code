function FConn_PreprocessingSPM12(structFile,funcFile,steps,opt)
...
%     end
    ...
    if ~isempty(strmatch('normalization',steps,'exact')),
        matlabbatch{end+1}.spm.spatial.normalise.est.subj.vol        = conn_prepend('c0',structFile{subj});
        ...
        matlabbatch{end+1}.spm.spatial.normalise.write.subj.def      = conn_prepend('y_c0',structFile{subj});
        ...
        matlabbatch{end+1}.spm.spatial.normalise.write.subj.def      = conn_prepend('y_c0',structFile{subj});
        temp = conn_prepend(prefix{1},funcFile{subj});
        matlabbatch{end}.spm.spatial.normalise.write.subj.resample   = cat(1,temp{:},conn_prepend(['mean',prefix1],cellstr(funcFile{subj}{1}{1})));
        matlabbatch{end}.spm.spatial.normalise.write.woptions.bb     = [-78 -112 -70; 78 76 85];
        matlabbatch{end}.spm.spatial.normalise.write.woptions.vox    = opt.vox*[1 1 1];
        matlabbatch{end}.spm.spatial.normalise.write.woptions.interp = 4;
