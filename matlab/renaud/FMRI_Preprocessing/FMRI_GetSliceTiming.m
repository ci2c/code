function slicetiming = FMRI_GetSliceTiming(Nslices,TRsec,acorder)

% Nslices = 40;
% acorder = 'interleaved';

if strcmp(acorder,'ascending')
    sliceorder = 1:1:Nslices;
elseif strcmp(acorder,'interleaved')
    sliceorder = [];
    space      = round(sqrt(Nslices));
    for k=1:space
        tmp        = k:space:Nslices;
        sliceorder = [sliceorder tmp];
    end
elseif strcmp(acorder,'descending')
    sliceorder = [Nslices:-2:1 Nslices-1:-2:1];
elseif strcmp(acorder,'interleavedAscending')
    sliceorder = [1:2:Nslices 2:2:Nslices];
else
    sliceorder = 1:1:Nslices;
end

% fnm = '/NAS/tupac/protocoles/healthy_volunteers/BIDS_DB/sub-T01S01/func/sub-T01S01_task-rest_run-01_bold.json';
% TRsec = jsonVal(fnm, '"RepetitionTime":')

sacq = TRsec/Nslices;

slicetiming = [0:sacq:TRsec-sacq];
slicetiming(sliceorder) = slicetiming;

% 
% function val = jsonVal(fnm, key)
% %read numeric values saved in JSON format
% % e.g. use 
% %    bidsVal1(fnm, '"RepetitionTime":') 
% %for file with 
% %   "RepetitionTime": 3,
% val = [];
% if ~exist(fnm, 'file'), return; end;
% txt = fileread(fnm);
% pos = strfind(txt,key);
% if isempty(pos), return; end;
% txt = txt(pos(1)+numel(key): end);
% pos = strfind(txt,'[');
% posComma = strfind(txt,',');
% if isempty(posComma), return; end; %nothing to do
% if isempty(pos) || (posComma(1) < pos(1)) %single value, not array
%     txt = txt(1: posComma(1)-1);
%     val = str2double(txt); %BIDS=sec, SPM=msec
%     return; 
% end;
% txt = txt(pos(1)+1: end);
% pos = strfind(txt,']');
% if isempty(pos), return; end;
% txt = txt(1: pos(1)-1);
% val = str2num(txt); %#ok<ST2NM> %BIDS=sec, SPM=msec
% %end jsonVal()