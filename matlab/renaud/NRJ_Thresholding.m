function resClust = NRJ_Thresholding(epipath,resClust,opt)
...
delete([epipath filesep 'Classes' filesep 'Thres_tMapsClass*.nii']);

st_write_nifti(AA,header,[epipath filesep 'Classes' filesep 'Thres_tMapsClass']);
...
if isfield(resClust,'COI')
    ...
    [a,b] = mkdir([epipath filesep 'Classes'],'COI');
    delete([epipath filesep 'Classes' filesep 'COI' filesep '*.*'])

    for pp = 1:length(selCOI)
        if(isunix)
            if selCOI(pp)<10
                unix(['cp ', fullfile(epipath,'Classes',['tMapsClass000',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['tMaps_',labelCOI{pp},'.nii'])]);
                unix(['cp ', fullfile(epipath,'Classes',['Thres_tMapsClass000',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['Thres_tMaps_',labelCOI{pp},'.nii'])]);
            else
                unix(['cp ', fullfile(epipath,'Classes',['tMapsClass00',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['tMaps_',labelCOI{pp},'.nii'])]);
                unix(['cp ', fullfile(epipath,'Classes',['Thres_tMapsClass00',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['Thres_tMaps_',labelCOI{pp},'.nii'])]);
            end
        elseif(ispc)
            if selCOI(pp)<10
                copyfile(fullfile(epipath,'Classes',['tMapsClass000',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['tMaps_',labelCOI{pp},'.nii']));
                copyfile(fullfile(epipath,'Classes',['Thres_tMapsClass000',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['Thres_tMaps_',labelCOI{pp},'.nii']));
            else
                copyfile(fullfile(epipath,'Classes',['tMapsClass00',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['tMaps_',labelCOI{pp},'.nii']));
                copyfile(fullfile(epipath,'Classes',['Thres_tMapsClass00',num2str(selCOI(pp)),'.nii ']), fullfile(epipath,'Classes','COI',['Thres_tMaps_',labelCOI{pp},'.nii']));
            end
