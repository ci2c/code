function FMRI_PlotPartCorrGraph(resClust,netw)
...
temp = resClust.stats.mtP;
mtP  = zeros(length(tabind),length(tabind));
for i = 1:length(tabind)
    mtP(i,:) = temp(tabind(i),tabind);
end
%     Pr        = ComputePvalue(mtR,nbframes,alpha);
%     mtRthres  = mtR;
%     mtRthres(Pr>alpha) = 0;

handleGraph1 = figure('Units','normalized','Name',['Partial Correlation Graph - Roi_level - ' name],'Resize','on','Position',[0.3 0.0302734 0.5 0.42]);
%nedica_visu_graph(mtRthres,coord,nameROI,[-1 1],[],maskClust);
nedica_visu_graph(mtP,coord,nameROI,[-1 1],[],maskClust);
rotate3d('on')
