function PRESTO_PreprocessingSPM12(anatFile,epiFile,nsubjects,steps,opt)
...
FUNCTIONAL_FILE = cellstr(conn_dir(epiFile));
structFile = cellstr(conn_dir(anatFile));
nsessions       = length(FUNCTIONAL_FILE)/nsubjects;
FUNCTIONAL_FILE = reshape(FUNCTIONAL_FILE,[nsubjects,nsessions]);
structFile = {structFile{1:nsubjects}};
...
for nsub=1:length(functionals)
	for nses=1:length(functionals{nsub})
		...
		if length(functionals{nsub}{nses})==1&&strcmp(tempc,'.nii')
			funcFile{nsub}{nses}=cellstr(spm_select('ExtFPList',tempa,['^',tempb,tempc],1:1e4)); 
		end;
	end;
end
...
%     nslices = V(1).dim(3);
    ...
    if ~isempty(strmatch('realignment',steps,'exact')),
        ...
        matlabbatch{end}.spm.spatial.realign.estwrite.eoptions.rtm   = 0;  % 0: first ; 1: mean
    ...
    if ~isempty(strmatch('coregistration',steps,'exact')),
        ...
        %matlabbatch{end}.spm.spatial.coreg.estimate.source = cellstr(funcFile{subj}{1}{1});
        temp = conn_prepend(prefix{1},funcFile{subj});
        %matlabbatch{end}.spm.spatial.coreg.estimate.other  = cat(1,temp{:},conn_prepend('mean',cellstr(funcFile{subj}{1}{1})));
