function compSelInfo = NRJ_CorsicaSpatialSelComp(sica_file,mask_file,opt)

% usage : compSelInfo = NRJ_CorsicaSpatialSelComp(sica_file,mask_file,opt)
%
% Inputs :
...
if (exist(list_files{1}(1,:)))&(strcmp(list_files{1}(1,end-2:end),'img'))
    ...
    for num_run = 1:length(list_files)
        courbes_N = FMRI_ReadNiftiTc(mask,list_files{num_run},0);
        for num_roi = 1:nb_rois
