function [data_lh_zscore,data_rh_zscore] = Surf_DataToZscore(file_lh,file_rh,method)
...
if(method==1)
    ...
    data_lh_zscore = niak_fisher(data_lh);
    data_lh_zscore(~isfinite(data_lh_zscore))=0;

    data_rh_zscore = niak_fisher(data_rh);
    data_rh_zscore(~isfinite(data_rh_zscore))=0;

    % save
    [pathstr,name,ext] = fileparts(file_lh);
    SurfStatWriteData(fullfile(pathstr,[name '.zscore']),data_lh_zscore,'b');

    [pathstr,name,ext] = fileparts(file_rh);
    SurfStatWriteData(fullfile(pathstr,[name '.zscore']),data_rh_zscore,'b');
...
elseif(method==2)
    ...
    % save
    [pathstr,name,ext] = fileparts(file_lh);
    SurfStatWriteData(fullfile(pathstr,[name '.zscore']),data_zscore(1:nb_lh)','b');

    [pathstr,name,ext] = fileparts(file_rh);
    SurfStatWriteData(fullfile(pathstr,[name '.zscore']),data_zscore(nb_lh+1:end)','b');
