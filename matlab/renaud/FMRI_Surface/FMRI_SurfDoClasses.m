function [numClass,resClust] = FMRI_SurfDoClasses(datalist,subjlist,prefix,resClust,outdir,opt)
...
s   = strfind(compsName_lh{1},'/');
ind = strfind(compsName_lh{1},subjlist{1});
pos = find(s==ind-1);
compsName_lh{1} = compsName_lh{1}(s(pos)+1:end);
compsName_rh{1} = compsName_rh{1}(s(pos)+1:end);
for k = 2:length(compsName_lh)
    s = strfind(compsName_lh{k},'/');
    compsName_lh{k} = compsName_lh{k}(s(pos)+1:end);
    compsName_rh{k} = compsName_rh{k}(s(pos)+1:end);
end
...
if length(subjlist) == 1
    ...
    [tmp, fnum] = read_curv(fullfile(datalist{1},['lh.' prefix '1.mgh']));
    nbleft      = length(tmp);
    
    for j = 1:nbCompSica
        [tmp, fnum]           = read_curv(fullfile(datalist{1},['lh.' prefix num2str(j) '.mgh']));
        tMaps(1:nbleft,j)     = tmp(:);
        [tmp, fnum]           = read_curv(fullfile(datalist{1},['rh.' prefix num2str(j) '.mgh']));
    ...
    if strcmp(typeThresHier,'auto')
        ...
        P             = FMRI_SelectionHierClass(hier,compsName_lh,compsName_rh,subjlist);
        nbclasses     = max(P);
        opt.nbclasses = nbclasses;
        disp(['number of classes : ',num2str(nbclasses)])
...
resClust.clusName_lh  = compsName_lh;
resClust.clusName_rh  = compsName_rh;
resClust.hier         = hier;
