function [ pval, peak, clus, clusid ] = SurfStatP_me( slm, mask, clusthresh );
...
if v==1
    pval.P=stat_threshold_surfstat(0,1,0,df,[10 slm.t(1)],[],[],[],slm.k,[],[],0);
    pval.P=pval.P(2);
    peak=[];
    clus=[];
    clusid=[];
...
if clusthresh<1
    thresh=stat_threshold_surfstat(0,1,0,df,clusthresh,[],[],[],slm.k,[],[],0);
else
    thresh=clusthresh;
end
...
if max(slm.t(1,mask))<thresh
    pval.P=stat_threshold_surfstat(resels,v,1,df,[10 slm.t],[],[],[],slm.k,[],[],0);
    pval.P=pval.P((1:v)+1);
    peak=[];
    clus=[];
    clusid=[];
    ...
    [pp,clpval]=stat_threshold_surfstat(resels,v,1,df,...
    ...
    if slm.k>1
        ...
        [pp,clpval]=stat_threshold_surfstat(conv(resels,sphere),Inf,1,df,...
            [],thresh,[10; clus.resels],[],[],[],[],0);
    end
    clus.P=clpval(2:length(clpval));
    pval.C=interp1([0; clus.clusid],[1; clus.P],clusid);        
end
tlim=stat_threshold_surfstat(resels,v,1,df,[0.5 1],[],[],[],slm.k,[],[],0);
tlim=tlim(2);
