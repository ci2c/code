function [P_final,R_final,U_final] = NRJ_SelHierClass(hier,compsName)
...
% disp('initialization...')
P_final = zeros(length(compsName),1);
R_final = [];
U_final = [];
numCOI = 1;
test = 1;
...
U_former = 0;
R_former = 1;
P_old = ned_hier2P(hier,nc,1);
while test
    nc = nc+1;
    %numCOI
    %disp(['numclass = ' num2str(nc)])
    P = ned_hier2P(hier,nc,1);
    R = [];
    U = [];
    for pp=1:nc
        AA = char(compsName{find(P == pp)});
        [R(pp),U(pp)] = ned_compute_scores(AA,hdr_data,listSelectedData);

    end
    %P2 = ned_hier2P(hier,nc-1,1);
    tt = 1;
    testm = 0;
