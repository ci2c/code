function NRJ_VolClusteringICAMaps(datalist,subjlist,outdir,pref_ica,optfunc,maskFile)
...
if (optfunc.mapMNI == 1)
    ...
    for k = 1:length(resClust.cois) 
        ...
        cmd = sprintf('mri_vol2surf --mov %s --mni152reg --hemi lh --o %s --trgsubject fsaverage --interp nearest --projfrac 0.5',map_file,curv_file);
        unix(cmd);
        
        curv_file = fullfile(outdir,['rh.Vol_Thres_tMapsClass_' num2str(coi) '.mgh']);
        cmd = sprintf('mri_vol2surf --mov %s --mni152reg --hemi rh --o %s --trgsubject fsaverage --interp nearest --projfrac 0.5',map_file,curv_file);
        unix(cmd);
