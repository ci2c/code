function [cluster_labels, objective_value] = sc_connectivity_bimat(A, B, num_clusters, ratio, ntrials, ev_flag)
% usage : [cluster_labels, objective_value] = sc_connectivity_bimat(A, B, num_clusters, [ratio, ntrials, ev_flag])
...
for j = 1 : n_ratio
    ...
    % Normalization
    sq_sum = sqrt(sum(V(:, 1:num_vec_A) .* V(:, 1:num_vec_A), 2)) + 1e-20;
    sq_sumb = sqrt(sum(Vb(:, 1:num_vec_B) .* Vb(:, 1:num_vec_B), 2)) + 1e-20;
    Ua = V(:, 1:num_vec_A) ./ repmat(sq_sum, 1, num_vec_A);
    Ub = Vb(:, 1:num_vec_B) ./ repmat(sq_sumb, 1, num_vec_B);
