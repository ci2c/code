...
if exist(filename)
    ...
    T = MakeDiffusion(Points, struct('Delta', 160, 'Threshold', 1e-3, 'Normalization', 'bimarkov'));
    OverT = MakeDiffusion(OverPoints, struct('Delta', 160, 'Threshold', 1e-3, 'Normalization', 'bimarkov'));
...
%    OverT = OverT^2;
    ...
    %Eigs = gs(ProjectedV);
    [Idxs DIdxs Eigs] = gsqr(ProjectedV, struct('StopN', N, 'StopPrecision', -1.0));
