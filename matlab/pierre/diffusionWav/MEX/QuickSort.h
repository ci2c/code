...
// have been accomplished using the STL library, but I don't like that piece
// of crap, so I just wrote it myself.
//
// Obviously, this is used to sort lists of (index, value) pairs for
// sparse vectors and matrices.
//
// Dependencies:        none
// Platforms:           should compile on just about every C++ compiler
...
void QuickSort(T *values, I *aux_data, int left, int right)
{
   ...
   while(stacklen) {
      stacklen--;
      left = stack[2*stacklen];
      right = stack[2*stacklen+1];
      ...
      if(right-left+1 > QSORT_MIN_PART) {
         ...
         while(1) {
            ...
            if(i<j) {
               ...
               // add new partitions to the stack
               stack[2*stacklen]= left;
               stack[2*stacklen+1] = j;
               stacklen++;
               stack[2*stacklen] = j+1;
               stack[2*stacklen+1] = right;
               stacklen++;
               break;
            }
         }
