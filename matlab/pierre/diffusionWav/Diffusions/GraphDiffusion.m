...
%                         'sqrtW.Wt'     : replaces W by sqrt(W.*W' (sqrt entrywise)
%                         'sqrtWWt'      : replaces W by sqrt(W*W') (sqrt entrywise)
%                         'sqrtWtW'      : replaces W by sqrt(W'*W) (sqrt entrywise)
...
%    [XIsTransposed]     : Points is N by D rather than D by N. Default: 0.
...
%     mm    12/10
%     mm    1/15/08
%     mm    8/4/08 added DistInfo containing matrices. To be finished...
%
%
...
for j=1:length(fn)
    ...
    if strcmpi(name, 'Display')
        G.Opts.Display = value;
    elseif strcmpi(name, 'kEigenVecs')
        G.Opts.kEigenVecs = value;
    elseif strcmpi(name, 'Epsilon')
        G.Opts.Epsilon = value;
    elseif strcmpi(name, 'Normalization')
        G.Opts.Normalization = value;
    elseif strcmpi(name, 'Metric')
        G.Opts.Metric = value;
    elseif strcmpi(name,'Distance')
        G.Opts.Distance = value;
    elseif strcmpi(name,'PCARadius')
        G.Opts.PCARadius = value;
    elseif strcmpi(name,'PCAtruncD')
        G.Opts.PCAtruncD = value;
    ...
    elseif strcmpi(name,'kNNAutotune')
        G.Opts.kNNAutotune = value;
    elseif strcmpi(name,'DistRestrict')
        G.Opts.DistRestrict = value;
    elseif strcmpi(name,'Symmetrization')
        G.Opts.Symmetrization = value;
    elseif strcmpi(name,'DontReturnDistInfo')
        G.Opts.DontReturnDistInfo = value;
    elseif strcmpi(name,'WeightFcn')
        G.Opts.WeightFcn = value;
    elseif strcmpi(name,'XIsTransposed'),
        G.Opts.XIsTransposed = value;
    elseif strcmpi(name, 'NoSelfLoop')
        G.Opts.NoSelfLoop = value;
    elseif strcmpi(name, 'BinaryWeights')
        G.Opts.BinaryWeights = value;  
    elseif strcmpi(name, 'ReturnAdjustedDistInfo')
        G.OPts.ReturnAdjustedDistInfo=value;
    else
        fprintf('GraphDiffusion.m: invalid option "%s" ignored.\n', name);
    end
end;
...
if G.Opts.DoRadiusSearch,
    lNumberOfGraphs = length(G.Opts.Radius);
else
    lNumberOfGraphs = length(G.Opts.kNN);
end;
...
for k = 1:lNumberOfGraphs,
    ...
    for i = N:-1:1,
        if G.Opts.DoRadiusSearch,
            ...
            if iscell(DistInfo.dists),
                lIdxs       = find(DistInfo.dists{i}<=G.Opts.Radius(k));
            else
                lIdxs       = find(DistInfo.dists(i,:)<=G.Opts.Radius(k));
            end;
            % We restrict to the kNN nearest neighbors
            if length(lIdxs)>G.Opts.kNN,
                lIdxs = lIdxs(1:G.Opts.kNN);
            end;
    ...
    if G.Opts.kNNAutotune > 0
        ...
        for j=N:-1:1,
            temp = sort(dists{j}, 'ascend');              % MM: This could be done once, at the cost of a bit of memory.
    ...
    if G.Opts.Epsilon(k)>0,
        W = sparse(rowidxs, colidxs,  distsmat, N, N, sum(count));
    else
        W = sparse(rowidxs, colidxs,  ones(size(distsmat)), N, N, sum(count));
    end;
    clear idxs rowidxs colidxs dists count;

    % Now take d(x,y) = d(x,y)*d(y,x) in order to symmetrize
    if ~isempty(strfind(G.Opts.Symmetrization,'W.Wt')),
        W = W .* W';
    elseif ~isempty(strfind(G.Opts.Symmetrization,'WWt')),
        W = W*W';
    elseif ~isempty(strfind(G.Opts.Symmetrization,'WtW')),
        W = W'*W;
    elseif ~isempty(strfind(G.Opts.Symmetrization,'W+Wt')),
        W = W + W';
    end;
    if ~isempty(strfind(G.Opts.Symmetrization,'sqrt')),
        W = sqrt(W);
    end;
