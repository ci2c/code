...
%
% IN:
%    cX         : DxN matrix of N points in R^D comprising the data set. MUST be of type double.
...
%                   XIsTransposed   : cX is passed transposed, i.e. it is M by N representing M points in N dimensions. Default: 0.
%                   [FastNNSearcher]: 'nn_search','ANNsearch','bruteforce','best' 
%                                       (i.e. tries nn_search first, then ANNsearch, then bruteforce). Default: 'best'.
...
%   MM:     4/9/2008  : added more options
%   MM:     6/1/2008  : added support for nn_search
%   MM:     6/8/2008  : some improvements and bug fixes
%   MM:     7/20/2008
%   MM:     8/11/2008
%
%

% MM: When using the fast nn searcher from TsTool, there may be a bug in that
% a point may not be among its nearest neighbors. To be checked.
...
if strcmpi(opts.FastNNSearcher,'best'),
    lUseTSTool = (exist('nn_prepare')==3); %if lUseTSTool, try, nn_prepare; catch lUseTSTool = false; end; end;
    lUseANNsearch = (exist('ANNsearch')==3); % Should to the following, but annoying output!   if lUseANNsearch, try, ANNsearch >> tmp.log; catch lUseANNsearch = false; end; end;
...
if opts.XIsTransposed==false,    [lDim,lNPts] = size(cX); else    [lNPts,lDim] = size(cX); end;

% If distances are not provided, or the metric is non-Euclidean, and no known fast nearest neighbor
% searcher is available, go brute force.
if ((nargin>=5) && ~isempty(metric)) || ...
        (~isempty(opts.DistInfo)) || ...
        ~(lUseANNsearch || lUseTSTool) || (size(cX,2)==1),           
    % Get the size of the query set
    if isempty(cXq), L = size(cX, 2); else L = size(cXq,2); end;
    % Allocate output variables
    if ~opts.ReturnAsArrays,
        idxs  = cell(1, L);
        if nargout>2, dists = cell(1, L);  end;
    else
        if NN>0,
            idxs(L,NN) = uint32(0);
            if nargout>2, dists(L,NN) = single(0); end;
        else
            idxs(L,L) = uint32(0);
            if nargout>2, dists(L,L) = single(Inf); end;
        end;
    end;
    % If distances are already provided
    if ~isempty(opts.DistInfo),
        lX_are_indices = (size(cX,2)==1);
        if lX_are_indices,
            [lX_sorted,lX_sorted_idxs] = sort(cX);
            lX_sorted = uint32(lX_sorted);
        end;
        for j=1:L,                                                          % Run through query points            
            if iscell(opts.DistInfo.dists), 
                alldists = opts.DistInfo.dists{cXq(j)};                         % MM: Pretty inefficient but ok
                allidxs  = opts.DistInfo.idxs{cXq(j)};
            else
                alldists = opts.DistInfo.dists(cXq(j),1:opts.DistInfo.count(cXq(j)));
                allidxs  = opts.DistInfo.idxs (cXq(j),1:opts.DistInfo.count(cXq(j)));
            end;
            if (~isempty(cX)) && (length(cX)<L),
                if lX_are_indices,
                    [lInTargetIdxs,lTmp,lInAllIdxs] = intersect_fast_sorted_first( lX_sorted, allidxs );
                    allidxs  = lX_sorted_idxs(lTmp);
                    alldists = alldists(lInAllIdxs);
                    [alldists,lSortedIdxs] = sort(alldists);
                    allidxs = allidxs(lSortedIdxs);
                else
                    fprintf('nrsearch: Warning: shouldn''t be here!'); % MM: debug purposes....
                end;
            end;
            lTmp = length(alldists);
            ordering = (1:lTmp)';                                           % The distances are already ordered
            if radius==0,
                lNN = min([NN,lTmp]);
                if lNN<NN,
                    fprintf('\n nrsearch: Warning: number of neighbors requested larger than that available from DistInfo.');  
                end;
            else  % If range search, restricts to points within distance radius
                lNN = lTmp;
                for k = 1:lTmp,
                    if alldists(ordering(k))>radius,
                        lNN = max([1,k-1]);
                        break;
                    end;
                end;
            ...
            % Save results
            if ~opts.ReturnAsArrays,
                idxs{j} = allidxs(ordering(1:lNN));
                if nargout>2,
                    dists{j} = alldists(1:lNN);
                end;
            else
                idxs(j,1:lNN) = allidxs(ordering(1:lNN));
                if nargout>2,
                    dists(j,1:lNN) = alldists(1:lNN);
                end;
            end;
        end;
    else
        % Get various size of objects involved
        if opts.XIsTransposed,cX = cX';end;                                     % MM: Waste of memory, could write a mirror version
        ...
        % If no cXq were specified, they're the cX itself
        if isempty(cXq),
            cXq = cX;
        end;

        alldists = zeros(lNPts, 1);

        % For each query point
        for j=1:L
            if ~((size(cXq,1)==1) && (size(cX,1)>1)),              
                % Do brute force search: compute distances to all the other points
                for i=1:lNPts
                    alldists(i) = metric(cXq(:,j), cX(:,i));
                end
            else
                for i=1:lNPts
                    alldists(i) = metric(cX(:,cXq(j)), cX(:,i));
                end
            end;
            % Sort the distances
            ...
            lNN = NN;
            % If range search, restricts to points within distance radius
            if radius>0,
            ...
            % Save results
            if ~opts.ReturnAsArrays,
                idxs{j} = ordering(1:lNN, 1);
                if nargout>2,
                    dists{j} = alldists(1:lNN, 1);
                end;
            else
                idxs(j,1:NN) = ordering(1:NN, 1);
                if nargout>2,
                    dists(j,1:NN) = alldists(1:lNN,1);
                end;
            end;
        end;
    end;
    ...
    lGoWithIdxs = (strcmpi(class(cXq),'uint32')); %MM: NEW!!!!! NOT BACKWARD COMPATIBLE.......  % ((size(cXq,1)==1) && (size(cX,1)>1)); 
    % Rmk by YM: Does it check whether cXq is index or data set depending on the dim of cX?
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if isempty(opts.DistRestrict),
        if ~lUseTSTool,
            ...
            if radius==0,
                ...
                [count, neighbors] = range_search( lX, NNInfo.Atria, double(lXq), radius,-1, opts.Tolerance );
                % The results are NOT sorted. Sort them
                for p = 1:size(neighbors,1),                            
                    if ~sum(ismember(neighbors{p,1},double(lXq))),         % MM: This seems unnecessary, and it's quite time consuming
                        neighbors{p,1} = [neighbors{p,1},double(lXq)];
                        neighbors{p,2} = [neighbors{p,2},0];
                        fprintf('\n\n\n\n nrsearch It finally happened!!!!!!!!!!\n\n\n\n'); %MM for debug purposes only
                    end;                                                   % end of unnecessary (???) part...
