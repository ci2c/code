function [tracts, selected] = select_fibers_fast2_gpu(Surf, tracts, label, thre, xyzFibers, ids)
% 
...
BOX = zeros(3, 2);
BOX(1,1) = min(Coords(1, :))-1;
BOX(1,2) = max(Coords(1, :))+1;
BOX(2,1) = min(Coords(2, :))-1;
BOX(2,2) = max(Coords(2, :))+1;
BOX(3,1) = min(Coords(3, :))-1;
BOX(3,2) = max(Coords(3, :))+1;

%% Keep fibers inside the box
disp('Keep fibers inside the box');
tic
if nargin < 5
...
BOOL = isin3Dbox_gpu(xyzFibers', BOX);
