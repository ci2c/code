function dT = get_dLET(W, k, Sigma, L_f, N1, N2)
% Usage : dT = get_dLET(W, k, Sigma, L_f, [N1], [N2])
%
% Inputs :
...
switch k
    case 1
        dT = ones(size(W));
    case 2
        dT = (1 + exp(-(W./(3.*Sigma)).^8) .* (8.*W - (3.*Sigma).^8)) ./ (3.*Sigma).^8;
    case 3
        dT = zeros(size(W));
        dT(:, 1:L_f) = ones(size(W(:, 1:L_f)));
    case 4
        dT = zeros(size(W));        
        dT(:, 1:L_f) = (1 + exp(-(W(:, 1:L_f)./(3.*Sigma)).^8) .* (8.*W(:, 1:L_f) - (3.*Sigma).^8)) ./ (3.*Sigma).^8;
    otherwise
        error('Invalid k');
end