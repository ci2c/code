...
if isempty(findobj(hToolbar_Children,'Tag', 'figPointTool')) 
  ...
  position = 9;
  separator = 'On';
  hMenus = [ hWindowLevelMenu, hPanZoomMenu,hROIToolMenu, hViewImageMenu, hRotateToolMenu ];
  if length(hMenus>0) 
...
if ~isempty(hToolbar)
	...
	old_ToolHandles  =     [Rot3D, ZoomO, ZoomI,WL,PZ,RT,MV,RotT];
	old_ToolEnables  = get([Rot3D, ZoomO, ZoomI,WL,PZ,RT,MV,RotT], 'Enable');
	old_ToolStates   = get([Rot3D, ZoomO, ZoomI,WL,PZ,RT,MV,RotT], 'State');
...
% open new figure
fig2 = openfig('PM_tool_figure','reuse');
