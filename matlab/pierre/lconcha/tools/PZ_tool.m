...
if isempty(findobj(hToolbar_Children,'Tag', 'figPanZoom')) 
  ...
  position = 9;
  separator = 'On';
  hMenus = [ hWindowLevelMenu, hROIToolMenu, hViewImageMenu, hPointToolMenu, hRotateToolMenu ];
...
if ~isempty(hToolbar)
	...
	old_ToolHandles  =     [Rot3D, ZoomO, ZoomI,WL,RT,MV,PM, RotT];
	old_ToolEnables  = get([Rot3D, ZoomO, ZoomI,WL,RT,MV,PM, RotT], 'Enable');
	old_ToolStates   = get([Rot3D, ZoomO, ZoomI,WL,RT,MV,PM, RotT], 'State');
