...
% Find handle for current image toolbar, menubar and objects
...
if ~isempty(hToolbar)  
	if isempty(findobj(hToolbar_Children,'Tag', 'figWindowLevel')) 
		...
		y = zeros(15,16,3);
		%%[y(:,:,2),y(:,:,3)] = meshgrid( linspace(0,1,16), linspace(0,1,15) );
		button_size_x= 16;
		y = repmat(linspace(0,1,button_size_x), [ 15 1 3]);
		
		hNewButton = uitoggletool(hToolbar);
		set(hNewButton, 'Cdata', y, ...
	...
	if ~isempty(hToolMenu) & isempty(findobj(hToolMenu,'Tag', 'menuWindowLevel')) 
		...
		position = 9;
		separator = 'On';
		hMenus = [hPanZoomMenu, hROIToolMenu, hViewImageMenu, hPointToolMenu, hRotateToolMenu];
...
if ~isempty(hToolbar)
	...
	old_ToolHandles  =     [Rot3D, ZoomO, ZoomI,PZ,RT,MV,PM,RotT];
	old_ToolEnables  = get([Rot3D, ZoomO, ZoomI,PZ,RT,MV,PM,RotT], 'Enable');
	old_ToolStates   = get([Rot3D, ZoomO, ZoomI,PZ,RT,MV,PM,RotT], 'State');
...
% open new figure
fig2 = openfig('WL_tool_figure','reuse');
...
set(findobj('Tag', 'WL_figure'), 'Closerequestfcn', 'closereq');
try 
	close(findobj('Tag','WL_figure'));
catch
	delete(findobj('Tag', 'WL_figure'));
end;
