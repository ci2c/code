...
% Last Modified by GUIDE v2.5 11-Sep-2006 13:37:09
...
if isempty(varargin)
    ...
    disp('USAGE: imlook3d(img) or To read an image use ImFile menu');
%     evalin('base','helpwin imlook3d');
...
Ax = Std; Ax.Units = 'Pixels'; Ax.ydir = 'reverse';
Ax.XLim = [.5 128.5]; Ax.YLim = [.5 128.5]; Ax.CLimMode = 'auto';
Ax.XTick = []; Ax.YTick = [];
...
% imlogo = imread(txt);
% Sz= size(imlogo);
% flogo = figure('Position',[10000 10000 Sz(2) + 4 Sz(1) + 4],'name','Imlook3D','numbertitle','off','menubar','none');
...
% pause(0.001);delete(flogo);clear imlogo
...
answer = inputdlg(prompt,dlg_title,num_lines,def);
if ~isempty(answer)
    voxdim = str2num(cell2mat(answer))';
