...
%            
    ...
    ygrille = defintm(6,1).Grille_reguliere_1;
    zgrille = defintm(6,1).Grille_reguliere_2;
    dy = defintm(6,1).Valeurs_deformation_1;
    dz = defintm(6,1).Valeurs_deformation_2;
%     
...
%% isocontours de d�formation radiale %%
    ...
    dr = sqrt(dy.^2+dz.^2);
    figure (3)
    imshow(Ima,[],'XData',ygrille(1,:),'YData',zgrille(:,1),'InitialMagnification','fit');hold on
    [C,h] = contour('v6',ygrille,zgrille,dr,0:0.2:2);
    map= jet(length(h));
    for n=1:length(map)
