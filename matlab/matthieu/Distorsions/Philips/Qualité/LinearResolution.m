function [LSFx,LSFy] = LinearResolution(name)
...
%     hold off;
    ...
    title('Profil type de LSF selon x');
    xlabel('Position x (en pixels)');
    ylabel('Niveau de gris du profil');
    hold off
    subplot(1,2,2)
    plot(ceil(Yhu):ceil(Yhd),PixelValuev,'b');hold on;
    plot(range_y,Valuev,'r');
    title('Profil type de LSF selon y');
    xlabel('Position y (en pixels)');
    ylabel('Niveau de gris du profil');
    hold off
...
%     subplot(1,2,1)
    ...
    title('Dérivée du profil de type LSF selon y et largeur à mi-hauteur');
    xlabel('Position x (en pixels)');
    ylabel('Dérivée de niveau de gris du profil');
    hold off
%     subplot(1,2,2)
...
%     L = length(range_xn);           % Length of signal
%  
%     NFFT = 2^nextpow2(L); % Next power of 2 from length of y
%     MTFh = fft(Derh_new,NFFT)/L;
...
%     plot(f,2*abs(MTFh(1:NFFT/2+1))) 
%     title('Single-Sided Amplitude Spectrum of MTFh(x)')
%     xlabel('Frequency (Hz)')
%     ylabel('|MTFh(f)|')
