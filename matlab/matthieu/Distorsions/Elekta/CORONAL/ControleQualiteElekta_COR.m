function ControleQualiteElekta_COR(dicomPath,sequence,threshold,maxDef)
%
%ControleQualiteElekta_COR allows user to quantify geometrical distorsions in
%three COR slices from a DICOM files directory.
%
%Requires: Image Processing Toolbox
%
%Usage: ControleQualiteElekta_COR(dicomPath,sequence,threshold,maxDef)
%   
%   Arguments :
...
for i = 1:length(NumSlices)
    ...
    if exist(name)~=0
        ...
        while ~preDetectOK
            pCg=[];
            if sequence=='T1'
                pCg = PreDetection_COR(name, sequence, threshold, maxDef);
        ...
        Cg = Detection_COR(name, sequence, threshold, maxDef);   
        ...
        [dX,dZ,dR,Xv,Zv,Ima1info,y0]= Deformations_COR(name,Pf,CG,Cgt,cp,maxDef);
    ...
    clear dX dZ dR Xv Zv Ima1info y0 R q dep Pf error CG Cth Cgt Cg name;
    close all;
    preDetectOK=0;
