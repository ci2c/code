...
if isempty(dicomList)
    dicomList = dir(fullfile(dicomPath,'A*'));
end
if isempty(dicomList)
    dicomList = dir(fullfile(dicomPath,'B*'));
end
if isempty(dicomList)
    dicomList = dir(fullfile(dicomPath,'C*'));
end
if isempty(dicomList)
    dicomList = dir(fullfile(dicomPath,'D*'));
end
...
for i = 1:length(NumSlices)
    ...
    if exist(name)~=0
        ...
        while ~preDetectOK
            pCg=[];
            if sequence=='T1'
                pCg = PreDetection_AXIAL(name, sequence, threshold, maxDef);
        ...
        Cg = Detection_AXIAL(name, sequence, threshold, maxDef);   
        ...
        [dX,dY,dR,Xv,Yv,Ima1info,z0]= Deformations_AXIAL(name,Pf,CG,Cgt,cp,maxDef);
    ...
    clear dX dY dR Xv Yv Ima1info z0 R q dep Pf error CG Cth Cgt Cg name;
    close all;
    preDetectOK=0;
