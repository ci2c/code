function FMRI_Preprocess_SPM8(data_path,TR,nslices,refslice,fwhm,coreg,acquisition,resampling)
...
%% REALIGN
%--------------------------------------------------------------------------
jobs{joblist(1)}.spatial{1}.realign{1}.estwrite.data = { editfilenames(f1,'prefix',prefix{1}) editfilenames(f2,'prefix',prefix{1}) }';
...
jobs{joblist(1)}.spatial{1}.realign{1}.estwrite.eoptions.rtm     = 0;
...
%--------------------------------------------------------------------------
jobs{joblist(2)}.temporal{1}.st.scans = { editfilenames(f1,'prefix',prefix{2}) editfilenames(f2,'prefix',prefix{2}) }';
...
if(resampling==0)
    ...
    if strcmp(coreg,'epi2anat')
        ...
        jobs{4}.spatial{1}.coreg{1}.estimate.source = editfilenames(f1(1,:),'prefix',prefix{4});
