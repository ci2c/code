...
% inDir = '/NAS/dumbo/protocoles/CogPhenoPark/Connectome/ANOVA';
% 
...
% nsamp0 = size(Y0,1);
% X     = zeros(nsamp0,5);
% X(:,1) = [ones(1,nsubj0) zeros(1,(nsamp0-nsubj0))]';
% X(:,2) = [zeros(1,nsubj0) ones(1,nsubj1) zeros(1,(nsamp0-nsubj0-nsubj1))]';
% X(:,3) = [zeros(1,nsubj0+nsubj1) ones(1,nsubj2) zeros(1,(nsamp0-nsubj0-nsubj1-nsubj2))]';
% X(:,4) = [zeros(1,nsubj0+nsubj1+nsubj2) ones(1,nsubj3) zeros(1,(nsamp0-nsubj0-nsubj1-nsubj2-nsubj3))]';
% X(:,5) = [zeros(1,nsubj0+nsubj1+nsubj2+nsubj3) ones(1,nsubj4) zeros(1,(nsamp0-nsubj0-nsubj1-nsubj2-nsubj3-nsubj4))]';
% 
% % Contrast
% contrast = ones(1,5);
% 
...
% save(fullfile(inDir,'nbs_ANOVA_G1toG5_Mfa.mat'),'GLM','STATS');
% clear GLM STATS;
% 
% % Group effect on MMD
% 
...
% GLM.perms       = 10000;
% STATS.test_stat = NBSglm(GLM);
% save(fullfile(inDir,'nbs_ANOVA_G1toG5_MMD.mat'),'GLM','STATS');
% clear GLM STATS;
% 
...
% GLM.perms       = 10000;
% STATS.test_stat = NBSglm(GLM);
% save(fullfile(inDir,'nbs_ANOVA_G1toG5_FD.mat'),'GLM','STATS');
% clear GLM STATS;
...
for j = 1:3
    ...
    clear GLM STATS NBS;
    load(fullfile(inDir,['nbs_ANOVA_G1toG5_' VCD{j} '.mat']),'GLM','STATS');
    STATS.alpha = alpha;
    ...
    %     NBSview(NBS);
        ...
        save(fullfile(inDir,['nbs_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '_' num2str(thresVal(k)) '.mat']),'NBS','GLM','STATS');
        ...
        for netw = 1:NBS.n
            ...
            fid = fopen(fullfile(inDir,['nbs_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '_' num2str(thresVal(k)) '_' num2str(netw) '.node']),'w');
            ...
            %edges
            FMRI_CreateEdgeFileBrainNet(fullfile(inDir,['nbs_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '_' num2str(thresVal(k)) '_' num2str(netw) '.edge']),double(C).*niak_vec2mat(STATS.test_stat(1,:)));
...
% %     NBSview(NBS);
% 
%     save(fullfile(inDir,['fdr_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '.mat']),'NBS','GLM','STATS');
% 
%     for netw = 1:NBS.n
% 
%         C = full(NBS.con_mat{netw});
...
%         nodes.labels = roiNames;
% 
%         fid = fopen(fullfile(inDir,['fdr_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '_' num2str(netw) '.node']),'w');
%         for cj = 1:size(nodes.coords,1)
%             % Coordinates
%             fprintf(fid,'%f ',nodes.coords(cj,1));
%             fprintf(fid,'%f ',nodes.coords(cj,2));
...
% 
%         %edges
%         FMRI_CreateEdgeFileBrainNet(fullfile(inDir,['fdr_ANOVA_G1toG5_' VCD{j} '_perm_p' num2str(alpha) '_' num2str(netw) '.edge']),double(C).*niak_vec2mat(STATS.test_stat(1,:)));
%     end
% end