...
mat_surf_parah = []; %matrice finale pour les données surfaciques du cortex enthorinal
mat_thick_parah = []; %Matrice finale pour les données d'épaisseur du cortex enthorinal
matt=[];
mats=[];
j=0; 
...
for i=1:length(liste)
    ...
    if fid ~= -1 %Si il arrive à aller chercher le fichier
      ...
      mat_surf_parah(j,1) = [mats];
      mat_thick_parah(j,1)=[matt];
