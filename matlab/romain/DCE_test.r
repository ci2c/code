...
library(oro.nifti)
setwd("/NAS/tupac/protocoles/perfusion/fs53//BATAILLE_RICHARD_2013-02-12/Perf/run01/")
img <- readNIfTI("DCE_normalized.nii.gz",reorient = FALSE)
setwd("/NAS/tupac/protocoles/perfusion/fs53//BATAILLE_RICHARD_2013-02-12/label/")
mask <-readNIfTI("roi_d.nii",reorient = FALSE)

library(readxl)
aif_mediane <- read_excel("/NAS/tupac/protocoles/perfusion/data/aif_mediane.xlsx")
time <- aif_mediane$time
aif <- aif_mediane$Mediane
