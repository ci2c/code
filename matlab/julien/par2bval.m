...
%         + 2014-03-06  - JD : Change dcm2nii version (07/2013) to fix bo
%                              position in the nii file
% ------------------------------------------------------------------------------%

function par2bval( par_file )

dcm2nii_path = '/home/global/mricron062013/';
...
% Preparation des temp
    ...
    % Creation nii
        if exist(rec_file,'file')==2
            cmd_convert = [dcm2nii_path,'dcm2nii -f n -g y -r n -x n -v n -o ',rep_temp,'/nii ', rep_temp,'/',fich,ext];
            system(cmd_convert);
    ...
    % Ecriture du nii
        ...
        if (length(dyn_a_supprimer)>=1  || pos_b0_init >1) 
          ...
          % Merge des dyn
           ...
           cmd_mv_nii =['mv ',fichier_nii,' ',rep,'/',fich,'.nii.gz'];
           system(cmd_mv_nii);
        end
