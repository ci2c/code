...
% virtual coil
P = zeros(nr_coils);
for i = 1:size(masked_data,2)
    P = P+masked_data(:,i) * pinv(masked_data(:,i));
end
[U,E,V] = svd(P);
A = U(:,1)';

% if equal_coil_weights
%     a = ones(1, nr_coils) ./ sqrt(nr_coils);
%     A = a.*exp(j.*angle(A));
% end
...
img = sum(conj(sensitivity).*data,3)./sum(abs(sensitivity).^2,3);
