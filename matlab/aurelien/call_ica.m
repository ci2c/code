function call_ica(basepath,resultdir)
clear all;
close all;
clc;

% basepath       = '/home/fatmike/aurelien/ASL/multi-TI/temoin01/temoin01_08/';
% resultdir      = '/home/fatmike/aurelien/ASL/multi-TI/temoin01/SICA_temoin08';
...
if ~exist([resultdir filesep 'sica.mat']) || flag.force == 1
    ...
    sica = script_sica(list_files,opt_sica);
...
if ~exist([resultdir filesep 'spatialComp']) || flag.force == 1
    if ~exist([resultdir filesep 'spatialComp' filesep 'wsica_comp0001.img']) || flag.force == 1
        ...
        for i=1:length(comps)
    		...
    		if i<10 
             	d.fname = [resultdir filesep 'spatialComp' filesep 'sica_comp000' num2str(comps(i)) '.img'];
            elseif i<100
                d.fname = [resultdir filesep 'spatialComp' filesep 'sica_comp00' num2str(comps(i)) '.img'];
            else
                d.fname = [resultdir filesep 'spatialComp' filesep 'sica_comp0' num2str(comps(i)) '.img'];
            end	
...
clear s a
d.fname = [resultdir filesep 'maskB.img'];
...
V.fname = [resultdir filesep selectionName filesep 'maskB_sica.img'];
st_write_analyze(double(maskB),V,V.fname);

opt_normalize.write.vox = sqrt(sum(d.mat(1:3,1:3).^2));
d.fname = [resultdir filesep selectionName filesep 'rawVoxSize.img'];
st_write_analyze(double(maskBrain),d,d.fname);
spm_write_sn(d.fname,params_normalize,opt_normalize.write);
delete([resultdir filesep selectionName filesep 'rawVoxSize.img'])
delete([resultdir filesep selectionName filesep 'rawVoxSize.hdr'])

PP = [resultdir filesep selectionName filesep 'wrawVoxSize.img'];
PP = strvcat(PP,[resultdir filesep selectionName filesep 'maskB_sica.img']);
...
if(isunix)
    unix(['mv ', resultdir filesep selectionName filesep 'rmaskB_sica.img ', resultdir filesep selectionName filesep 'maskB.img']);
    unix(['mv ', resultdir filesep selectionName filesep 'rmaskB_sica.hdr ', resultdir filesep selectionName filesep 'maskB.hdr']);
elseif(ispc)
    movefile([resultdir filesep selectionName filesep 'rmaskB_sica.img'],[resultdir filesep selectionName filesep 'maskB.img']);
