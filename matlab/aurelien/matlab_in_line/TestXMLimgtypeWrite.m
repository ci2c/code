%==========================================================================
% Script to read PRIDE XML file and write it back
% Author        Amol Pednekar
%               MR Clinical Science
%               Philips medical systems
%               8/25/2009
%==========================================================================
function  varargout = TestXMLimgtypeWrite(varargin)
...
if (strcmp(run_location,consolePath)) % On console
    % Determine EWS or Scanner
    if (exist('G:\patch\PRIDE','dir')==7) % Scanner
        root_location = 'G';
        cd('G:\patch\PRIDE');
    elseif (exist('D:\patch\PRIDE','dir')==7) % EWS
        root_location = 'D';
        cd('D:\patch\PRIDE');
    end
    fprintf('%s Recognized console ... \n',pwd);
    p = inputParser;
...
%     pause;
    try % Trap errors
        p.parse(varargin{1});
        fprintf('ROID is %s \n',p.Results.roid);
        source_location      = strcat(root_location,':\patch\PRIDE\tempinputseries');
        destination_location = strcat(root_location,':\patch\PRIDE\tempoutputseries');
        ...
        %         pause;
        xmlFileName = strcat(root_location,':\patch\PRIDE\tempinputseries\',xmlFileName);
...
% Write REC file
MaxSl = seriesStruct.Max_No_Slices;
...
for NoSl = 1:MaxSl
    for NoPh = 1:MaxPh
        ...
        imageStruct.Index = ((NoSl-1)*NoTypes*MaxPh)+NoPh-1;
        imageStruct.Type = 'M';
        ...
        fclose(Fid);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Image type 'I' to add thresh image
        ...
        fclose(Fid);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Image type 'R' to add thresh image
        imageStruct.Index = imageStruct.Index+1;
        imageStruct.Type = 'R';
        % Add image information to XML
        outTree = addImageToXML(outTree,imageStruct);
