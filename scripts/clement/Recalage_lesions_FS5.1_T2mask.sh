...
DirL=/NAS/tupac/protocoles/Strokdem/Lesions/72H
...
do  
  ...
  then
    ...
    brainmask=/NAS/tupac/protocoles/Strokdem/FS5.1_T2mask/$num/mri/brainmask.mgz
    ...
    if [ ! -e ${lesions} ]; then
      mri_label2vol --label ${f}/*.label --o $lesions --identity --temp ${f}/dwi_b_0001.nii.gz
    fi
