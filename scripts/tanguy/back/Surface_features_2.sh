...
then
	echo ""
	echo "Usage: Surface_features_blur_5_10_15_20.sh -sd <SUBJECTS_DIR>  -subj <SUBJ_ID>  [-comp] [-im <im1> <im2> <...>]"
	echo ""
	...
	echo ""
	echo ""
	echo "  -im                              : images used (ex : T1 FLAIR PET DTI ASL ...)"
	echo "without specification of imaging, all images will be used (depending on the version of the script)"
	echo ""
...
# Possibility to use T1 FLAIR & PET
#
#if no indication, all the images are used.
#
# to add another one, add
#
# if [ "name of image" == "$item" ]; then
#        echo "name of image used"
#	Cname_of_image=1
#	echo "Cname_of_image = $Cname_of_image"
# fi
...
# if T1 is necessary, test its existence

if [ $CT1 -eq 1 ]
then

	# Test if T1 is present (PT1=0 or 1)
...
#Test on PT1 & CT1 -> running T1


if [ $CT1 -eq 1 ] && [ $PT1 -eq 1 ]
then
...
# (close conditions of using & existing)
fi
...
# if FLAIR is necessary, test its existence
...
# Test if FLAIR is present (PFLAIR=0 or 1)
...
# Need T1 in NIFTI format -> need T1.nii

	if [ ! -f ${DIR}/epilepsy/T1.nii ]
	then
		mri_convert ${DIR}/mri/T1.mgz ${DIR}/epilepsy/T1.nii
	fi
fi



#Test on PFLAIR & CFLAIR -> running for FLAIR


if [ $CFLAIR -eq 1 ] && [ $PFLAIR -eq 1 ]
then
...
rm -f ${DIR}/epilepsy/flair_nuc_2_dxyz.mnc ${DIR}/epilepsy/flair_nuc_2_blur.mnc ${DIR}/epilepsy/flair_nuc.mnc  ${DIR}/epilepsy/flair_nuc_byte.mnc
...
# (close conditions for using & existing)
fi
...
# if PET is necessary, test its existence


if [ $CPET -eq 1 ]
then

# Test if PET is present (PFLAIR=0 or 1)

	PPET=1

	if [ ! -f ${DIR}/epilepsy/pet_las.nii ]
	then
		echo "need a pet file named pet_las.nii _ not working with pet"
		PPET=0	
	fi
fi
...
#Test on PPET & CPET -> running for PET

if [ $CPET -eq 1 ] && [ $PPET -eq 1 ]
then
...
mri_convert ${DIR}/epilepsy/T1.nii ${DIR}/epilepsy/T1_las.nii --out_orientation LAS
...
# control if T1 is used and present

if [ $CT1 -eq 1 ] && [ $PT1 -eq 1 ]
then

echo "running T1 for FWHM = ${FWHM}"
...
# close condition for using and existing
fi
...
# control if FLAIR is used and present

if [ $CFLAIR -eq 1 ] && [ $PFLAIR -eq 1 ]
then

echo "running FLAIR for FWHM = ${FWHM}"
...
# control if PET is used and present

if [ $CPET -eq 1 ] && [ $PPET -eq 1 ]
then

echo "running PET for FWHM = ${FWHM}"
