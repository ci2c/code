...
then
	echo ""
	echo "Usage: PRESTO_SurfICAClustering.sh  -sd <path>  -f <file>  -o <outdir>  -pref <name>  -ica <folder>  -thres <value>  -clusSize <value>  -tr <value> "
	echo ""
	echo "  -sd                           : freesurfer subjects directory "
	echo "  -f                            : list of subjects (.txt) "
	...
	echo "  -tr                           : TR value "
	echo ""
	echo "Usage: PRESTO_SurfICAClustering.sh  -sd <path>  -f <matFile>  -o <outdir>  -pref <name>  -ica <folder>  -thres <value>  -clusSize <value>  -tr <value> "
	echo ""
	exit 1
fi
...
do
	...
	-f)
		index=$[$index+1]
		eval subjFile=\${$index}
		echo "list of subjects : ${subjFile}"
		;;
	-o)
		index=$[$index+1]
		eval output=\${$index}
...
/usr/local/matlab11/bin/matlab -nodisplay <<EOF
	...
	subjlist = textread('${subjFile}','%s%*[^\n]');
	
	datalist  = {};
	for k = 1 : length(subjlist)    
	    datalist{k} = fullfile('${fsdir}',subjlist{k},'${icafolder}');       
	end
