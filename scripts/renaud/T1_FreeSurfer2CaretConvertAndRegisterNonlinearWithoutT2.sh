...
set -e
echo -e "\n"
echo -e "\n START: T1_FS2CaretConvertRegisterNonlinearWithoutT2.sh"
...
echo "set -- --StudyFolder="$StudyFolder" \
	  ...
	  --FreeSurferFolder="$FreeSurferFolder" \
	  --FreeSurferInput="$FreeSurferInput" \
	  --T1wImage="$T1wImage" \
	  --SurfaceAtlasDIR="$SurfaceAtlasDIR" \
	  --HighResMesh="$HighResMesh" \
	  --LowResMeshes="$LowResMeshes" \
	  --AtlasTransform="$AtlasTransform" \
      --InverseAtlasTransform="$InverseAtlasTransform" \
	  --AtlasSpaceT1wImage="$AtlasSpaceT1wImage" \
	  --AtlasSpaceT1wImage="$AtlasSpaceT1wImage" \
	  --T1wImageBrainMask="$T1wImageBrainMask" \
	  --FreeSurferLabels="$FreeSurferLabels" \
	  --GrayordinatesSpaceDIR="$GrayordinatesSpaceDIR" \
	  --SubcorticalGrayLabels="$SubcorticalGrayLabels" \
	  --RegName="$RegName" \
	  --"
...
echo "STRINGII=${STRINGII}"

#Create CIFTI Files
echo "Create CIFTI Files"
...
for STRING in "$T1wFolder"/"$NativeFolder"@"$AtlasSpaceFolder"/"$NativeFolder"@native "$AtlasSpaceFolder"/"$NativeFolder"@"$AtlasSpaceFolder"/"$NativeFolder"@native "$AtlasSpaceFolder"@"$AtlasSpaceFolder"@"$HighResMesh"k_fs_LR ${STRINGII} ; do
	...
	echo "FolderI=${FolderI}"
	echo "FolderII=${FolderII}"
	echo "Mesh=${Mesh}"
