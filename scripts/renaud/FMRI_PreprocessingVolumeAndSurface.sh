...
then
	echo ""
	echo "Usage: FMRI_PreprocessingVolumeAndSurface.sh -sd <SUBJECTS_DIR>  -subj <SUBJ_ID>  -epi <fmri_file>  -o <path>  [-fwhmsurf <value>  -fwhmvol <value>  -acquis <name>  -rmframe <value>  -doGMS  -tr <value> -toMNI  -doCompCor  -doFilt  -doAAL  -doCraddock  -doStriatum  -doSPMNorm  -doANTSNorm  -doSeg  -doSBA  -oldNorm  -v <FSVersion> ]"
	echo ""
...
if [ "${FS_VERSION}" == "5.1" ]
then
	echo "Freesurfer version 5.1"
	export FREESURFER_HOME=${Soft_dir}/freesurfer5.1/
	. ${FREESURFER_HOME}/SetUpFreeSurfer.sh
elif [ "${FS_VERSION}" == "5.0" ]
then
	echo "Freesurfer version 5.0"
	export FREESURFER_HOME=${Soft_dir}/freesurfer/
	. ${FREESURFER_HOME}/SetUpFreeSurfer.sh

elif [ "${FS_VERSION}" == "6.0" ]
then
	echo "Freesurfer version 6.0"
	export FREESURFER_HOME=${Soft_dir}/freesurfer6_0/
	. ${FREESURFER_HOME}/SetUpFreeSurfer.sh	
else
        echo "Freesurfer version 5.3"
	export FREESURFER_HOME=${Soft_dir}/freesurfer5.3/
	. ${FREESURFER_HOME}/SetUpFreeSurfer.sh
fi


if [ ! -d ${SUBJECTS_DIR}/fsaverage5 ]
then
...
cmd="fslmerge -t ${DIR}/${outdir}/run01/epi.nii ${DIR}/${outdir}/run01/tmp/epi_*"
echo $cmd; eval $cmd
cmd="gunzip -f ${DIR}/${outdir}/run01/*.gz"
echo $cmd; eval $cmd
cmd="rm -rf ${DIR}/${outdir}/run01/tmp"
echo $cmd; eval $cmd
...
then
	...
	cmd="mri_convert ${DIR}/${outdir}/run01/epi.nii ${DIR}/${outdir}/template.nii --frame 0"
	echo $cmd; eval $cmd
	cmd="mri_convert ${DIR}/${outdir}/run01/epi.nii ${DIR}/${outdir}/run01/template.nii --mid-frame"
	echo $cmd; eval $cmd
...
then
	cmd="mc-afni2 --i ${DIR}/${outdir}/run01/epi.nii --t ${DIR}/${outdir}/run01/template.nii --o ${DIR}/${outdir}/run01/repi.nii --mcdat ${DIR}/${outdir}/run01/repi.mcdat"
	echo $cmd;eval $cmd
	# Making external regressor from mc params
	cmd="mcdat2mcextreg --i ${DIR}/${outdir}/run01/repi.mcdat --o ${DIR}/${outdir}/run01/mcprextreg"
	echo $cmd;eval $cmd
...
/usr/local/matlab/bin/matlab -nodisplay <<EOF
	...
	elseif strcmp('${acquis}','interleavedAscending')
...
cp ${DIR}/${outdir}/run01/arepi.nii ${DIR}/${outdir}/run01/arepi_sm${fwhmvol}_al.nii.gz
fslmaths ${epipre} -Tmean ${DIR}/${outdir}/run01/mean_arepi.nii
gunzip -f ${DIR}/${outdir}/run01/mean_arepi.nii.gz
cp ${DIR}/${outdir}/run01/mean_arepi.nii ${DIR}/${outdir}/run01/mean_arepi_al.nii
...
then
	...
	#Smoothing ${DIR}/${outdir}/run01/arepi_al.nii
...
then
    ...
    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/arepi_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o arepi_al_MNI152_2mm
    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/mean_arepi_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o mean_arepi_al_MNI152_2mm
    
    #Smoothing ${DIR}/${outdir}/run01/arepi_al_MNI152_2mm.nii.gz
...
cmd="gunzip ${DIR}/${outdir}/T1_las.nii.gz"
echo $cmd; eval $cmd

cmd="gunzip ${DIR}/${outdir}/y_T1_las.nii.gz"
echo $cmd; eval $cmd
...
then	
	...
	if ${oldNorm}==0
	  ...
	  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.template = {[t '/toolbox/OldNorm/EPI.nii,1']};
...
EOF
	...
	#Smoothing ${DIR}/${outdir}/run01/warepi_al.nii
...
/usr/local/matlab/bin/matlab -nodisplay <<EOF
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(1).tpm = {[t '/tpm/TPM.nii,1']};
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(2).tpm = {[t '/tpm/TPM.nii,2']};
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(3).tpm = {[t '/tpm/TPM.nii,3']};
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(4).tpm = {[t '/tpm/TPM.nii,4']};
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(5).tpm = {[t '/tpm/TPM.nii,5']};
	...
	matlabbatch{end}.spm.spatial.preproc.tissue(6).tpm = {[t '/tpm/TPM.nii,6']};
...
/usr/local/matlab/bin/matlab -nodisplay <<EOF
		...
		if ${oldNorm}==0
		  ...
		  matlabbatch{end}.spm.tools.oldnorm.est.subj.wtsrc        = '';
		  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.template = {[t '/toolbox/OldNorm/T1_las.nii,1']};
		  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.weight   = '';
		  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.smosrc   = 8;
		  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.smoref   = 0;
		  matlabbatch{end}.spm.tools.oldnorm.est.eoptions.regtype  = 'mni';
...
if [ $DoSBA -eq 1 ] && [ ! -f ${DIR}/${outdir}/run01/arepi.sm${fwhmsurf}.fsaverage5.rh.nii ]
then
	...
	mri_binarize --i ${DIR}/${outdir}/run01/masks/brain.fsaverage.rh.nii --min .00001 --o ${DIR}/${outdir}/masks/brain.fsaverage.rh.nii
	...
	mri_binarize --i ${DIR}/${outdir}/run01/masks/brain.fsaverage5.rh.nii --min .00001 --o ${DIR}/${outdir}/masks/brain.fsaverage5.rh.nii
...
then
	...
	opt = struct('flag_compcor',0,'compcor',struct(),'nb_vol_min',40,'flag_scrubbing',0,'thre_fd',0.8,'flag_slow',1,...
...
EOF
	...
	then
	    ...
	    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/carepi_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o carepi_al_MNI152_2mm
...
then
	...
	CMD="cp ${DIR}/${outdir}/run01/arepi.nii ${DIR}/${outdir}/run01/arepi_sc.nii"
	echo $CMD;eval $CMD
	CMD="
	% Load Matlab Path
	cd ${HOME}
...
EOF
	...
	then
	    ...
	    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/carepi_sc_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o carepi_sc_al_MNI152_2mm
...
/usr/local/matlab/bin/matlab -nodisplay <<EOF
	...
	addpath(p);
	addpath('/home/global/matlab_toolbox/spm12/')
	addpath('/home/global/matlab_toolbox/niak-boss-0.12.18/')
...
EOF
	...
	then
	    ...
	    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/fcarepi_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o fcarepi_al_MNI152_2mm
...
EOF
	...
	then
	    ...
	    FMRI_ApplyAntsWarp.sh -epi ${DIR}/${outdir}/run01/fcarepi_sc_al.nii -temp ${TEMPLATE2} -pref ${DIR}/${outdir}/norm_mni152 -o fcarepi_sc_al_MNI152_2mm
...
# Compress data
gzip -f ${DIR}/${outdir}/*.nii
gzip -f ${DIR}/${outdir}/masks/*.nii
gzip -f ${DIR}/${outdir}/run01/*.nii
