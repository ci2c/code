...
then
	echo ""
	echo "Usage: T1_FS2CaretWithT2.sh  -sd <subjects_dir>  -subj <subject>  -t1 <t1_image>  -t2 <t2_image>  [-transf <name>  -invtransf <name>  -brainmask <name>  -atlast1 <name>  -atlast2 <name> ]  "
	echo ""
	echo "  -sd                         : SUBJECTS_DIR folder "
	echo "  -subj                       : Subject id "
	echo "  -t1                         : t1 image (.nii.gz) "
	echo "  -t2                         : t2 image (.nii.gz) "
	echo ""
	echo "Options "
	echo "  -transf                     : atlas transformation (Default: acpc_dc2standard)"
	echo "  -invtransf                  : atlas inverse transformation (Default: standard2acpc_dc)"
	echo "  -brainmask                  : T1 brain mask (Default: brainmask_fs)"
	echo "  -atlast1                    : T1 in atlas space (Default: T1w_restore)"
	echo "  -atlast2                    : T2 in atlas space (Default: T2w_restore)"
	echo ""
	echo "Usage: T1_FS2CaretWithT2.sh  -sd <subjects_dir>  -subj <subject>  -t1 <t1_image>  -t2 <t2_image>  [-transf <name>  -invtransf <name>  -brainmask <name>  -atlast1 <name>  -atlast2 <name> ] "
	echo ""
	exit 1
...
do
	...
	-transf)
		index=$[$index+1]
		eval AtlasTransform=\${$index}
		echo "atlas transformation : ${AtlasTransform}"
		;;
	-invtransf)
		index=$[$index+1]
		eval InverseAtlasTransform=\${$index}
		echo "inverse atlas transformation : ${InverseAtlasTransform}"
		;;
	-brainmask)
		index=$[$index+1]
		eval T1wImageBrainMask=\${$index}
		echo "T1w brain mask : ${T1wImageBrainMask}"
		;;
	-atlast1)
		index=$[$index+1]
		eval AtlasSpaceT1wImage=\${$index}
		echo "T1w in atlas space : ${AtlasSpaceT1wImage}"
		;;
	-atlast2)
		index=$[$index+1]
		eval AtlasSpaceT2wImage=\${$index}
		echo "T2w in atlas space : ${AtlasSpaceT2wImage}"
		;;
	-*)
		eval infile=\${$index}
...
if [ -z "${CARET7DIR}" ]; then
	echo "CARET7DIR environment variable must be set"
fi
...
T1wImage="${T1wInputImage%%.*}"
echo "T1wImage: ${T1wImage}"

T2wImage="${T2wInputImage%%.*}"
...
#T1wImageBrainMask=${T1wFolder}/${T1wImageBrainMask}
echo "T1wImageBrainMask: ${T1wImageBrainMask}"

#AtlasSpaceT1wImage=${AtlasSpaceFolder}/${AtlasSpaceT1wImage}
AtlasSpaceT1wImage=$(basename "$AtlasSpaceT1wImage")
AtlasSpaceT1wImage="${AtlasSpaceT1wImage%%.*}"
echo "AtlasSpaceT1wImage: ${AtlasSpaceT1wImage}"

#AtlasSpaceT2wImage=${AtlasSpaceFolder}/${AtlasSpaceT2wImage}
AtlasSpaceT2wImage=$(basename "$AtlasSpaceT2wImage")
AtlasSpaceT2wImage="${AtlasSpaceT2wImage%%.*}"
...
echo ""
echo "START: T1_FS2CaretWithT2.sh"
echo ""

# Find c_ras offset between FreeSurfer surface and volume and generate matrix to transform surfaces
echo "Find c_ras offset between FreeSurfer surface and volume and generate matrix to transform surfaces"
...
# Create T1 after bias correction from freesurfer
echo ""
echo "Create FreeSurfer T1"
mri_convert -rt cubic -rl "$T1wFolder"/"$T1wImage".nii.gz "$FreeSurferFolder"/mri/T1.mgz "$T1wFolder"/"$T1wImage"_fs_1mm.nii.gz
applywarp --rel --interp=spline -i "$T1wFolder"/"$T1wImage"_fs_1mm.nii.gz -r "$T1wFolder"/"$T1wImage" --premat=$FSLDIR/etc/flirtsch/ident.mat -o "$T1wFolder"/"$T1wImage"_fs.nii.gz
fslmaths "$T1wFolder"/"$T1wImage"_fs -mas "$T1wFolder"/"$T1wImageBrainMask" "$T1wFolder"/"$T1wImage"_fs_brain
...
echo ""
echo "START: Loop through left and right hemispheres"
echo ""
for Hemisphere in L R ; do
	...
	echo ""
	echo "Convert and volumetrically register white and pial surfaces makign linear and nonlinear copies, add each to the appropriate spec file"
	...
	# Create midthickness by averaging white and pial surfaces and use it to make inflated surfaces
	echo ""
	echo "Create midthickness by averaging white and pial surfaces and use it to make inflated surfaces"
	...
	echo ""
	echo "Convert original and registered spherical surfaces and add them to the nonlinear spec file"
	...
	echo ""
	echo "Add more files to the spec file and convert other FreeSurfer surface data to metric/GIFTI including sulc, curv, and thickness"
	...
	echo ""
	echo "Thickness specific operations"
	...
	echo ""
	echo "End main native mesh processing"
	echo ""
	# End main native mesh processing

	
	#====================================================================================
	# Atlas Mesh Processing
	#====================================================================================

	echo ""
	echo "Atlas mesh processing"
	echo ""

	# Copy Atlas Files
	echo ""
	echo "Copy Atlas Files"
	echo ""
	...
	echo ""
	echo "Concatenate FS registration to FS --> FS_LR registration"
	echo ""
	...
	echo ""
	echo "Make FreeSurfer Registration Areal Distortion Maps"
	echo ""
...
#	${CARET7DIR}/wb_command -surface-distortion "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".sphere.native.surf.gii "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".sphere.reg.reg_LR.native.surf.gii "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".Strain_FS.native.shape.gii
...
#  	rm "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".Strain_FS.native.shape.gii
	...
	if [ ${RegName} = "MSMSulc" ] ; then
		echo ""
		echo "run MSMSulc folding-based registration to FS_LR initialized with FS affine"
		echo ""
...
#    		rm "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".Strain_MSMSulc.native.shape.gii
		...
		echo ""
		echo "do not run MSMSulc folding-based registration to FS_LR initialized with FS affine"
		echo ""
	...
	echo ""
	echo "Ensure no zeros in atlas medial wall ROI"
	echo ""
	...
	echo ""
	echo "Deform surfaces and other data according to native to folding-based registration selected above"
	echo ""
	...
	echo ""
	echo "Metric resample: apply correction distortion"
	echo ""
...
#		${CARET7DIR}/wb_command -metric-resample "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".StrainR_MSMSulc.native.shape.gii ${RegSphere} "$AtlasSpaceFolder"/"$Subject"."$Hemisphere".sphere."$HighResMesh"k_fs_LR.surf.gii ADAP_BARY_AREA "$AtlasSpaceFolder"/"$Subject"."$Hemisphere".StrainR_MSMSulc."$HighResMesh"k_fs_LR.shape.gii -area-surfs "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".midthickness.native.surf.gii "$AtlasSpaceFolder"/"$Subject"."$Hemisphere".midthickness."$HighResMesh"k_fs_LR.surf.gii
	...
	for LowResMesh in ${LowResMeshes} ; do
		...
		echo ""
		echo "Create downsampled fs_LR spec files"
		echo ""
		for Surface in white midthickness pial ; do
...
#			${CARET7DIR}/wb_command -metric-resample "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".StrainR_MSMSulc.native.shape.gii ${RegSphere} "$AtlasSpaceFolder"/fsaverage_LR"$LowResMesh"k/"$Subject"."$Hemisphere".sphere."$LowResMesh"k_fs_LR.surf.gii ADAP_BARY_AREA "$AtlasSpaceFolder"/fsaverage_LR"$LowResMesh"k/"$Subject"."$Hemisphere".StrainR_MSMSulc."$LowResMesh"k_fs_LR.shape.gii -area-surfs "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere".midthickness.native.surf.gii "$AtlasSpaceFolder"/fsaverage_LR"$LowResMesh"k/"$Subject"."$Hemisphere".midthickness."$LowResMesh"k_fs_LR.surf.gii
		...
		echo ""
		echo "Create downsampled fs_LR spec file in structural space"
		echo ""
		for Surface in white midthickness pial ; do
...
echo ""
echo "END: Loop through left and right hemispheres"
echo ""


echo ""
echo "START: Last steps for high res"
echo ""
...
#		${CARET7DIR}/wb_command -cifti-palette "$Folder"/"$Subject".StrainR_MSMSulc."$Mesh".dscalar.nii MODE_USER_SCALE "$Folder"/"$Subject".StrainR_MSMSulc."$Mesh".dscalar.nii -pos-user 0 1 -neg-user 0 -1 -interpolate true -palette-name ROY-BIG-BL -disp-pos true -disp-neg true -disp-zero false
	...
	echo "Remove BA because it doesn't convert properly"
	for Map in aparc aparc.a2009s ; do #Remove BA because it doesn't convert properly
...
echo ""
echo "END: Create CIFTI Files"
echo ""

echo ""
echo "END: Last steps for high res"
echo ""

echo ""
echo "START: Last steps for low res"
echo ""
...
echo "Add CIFTI Maps to Spec Files"
