...
then
	echo ""
	echo "Usage: DTI_Preprocessing.sh  -dtipos <path>  -dtineg <path>  -o <path>  -echospacing <value>  [-denoise  -pedir <value>  -b0dist <value>  -b0max <value>  -onlypos  -t1 <t1_nifti>  -t1brain <t1brain_nifti>  -brainmask <mask_nifti>  -t1restore <nifti>  -bias <nifti>  -fs <folder>  -subj <name>]"
	echo ""
	...
	echo ""
	echo "Usage: DTI_Preprocessing.sh  -dtipos <path>  -dtineg <path>  -o <path>  -echospacing <value>  [-denoise  -pedir <value>  -b0dist <value>  -b0max <value>  -onlypos  -t1brain <t1brain_nifti>  -brainmask <mask_nifti>  -t1restore <nifti>  -bias <nifti>  -fs <folder>  -subj <name>]"
	exit 1
fi
...
DTI_TransposeGradientFiles.sh ${outdir}/rawdata/${basePos}_${Pos_count}.bval ${outdir}/rawdata/${basePos}_${Pos_count}.bvec
DTI_TransposeGradientFiles.sh ${outdir}/rawdata/${baseNeg}_${Neg_count}.bval ${outdir}/rawdata/${baseNeg}_${Neg_count}.bvec
...
echo "################################################################################################"
echo "##                                  REGISTRATION DTI-T1 "
echo "################################################################################################"


if [ $T1wImage = "NONE" ] ; then echo ""; else

	echo "do DTI-T1 coregistration"
	regdir=${outdir}/reg

	echo "DTI_DiffusionToStructural.sh -t1 ${T1wImage} -t1brain ${T1wBrainImage} -t1restore ${T1wRestoreImage} -bias ${BiasField} -dti ${datadir}/data -bval ${datadir}/bvals -bvec ${datadir}/bvecs -oreg ${regdir} -o T1 -brainmask ${InputBrainMask}"

	DTI_DiffusionToStructural.sh -t1 ${T1wImage} -t1brain ${T1wBrainImage} -t1restore ${T1wRestoreImage} -bias ${BiasField} -dti ${datadir}/data -bval ${datadir}/bvals -bvec ${datadir}/bvecs -oreg ${regdir} -o T1 -brainmask ${InputBrainMask}
