...
#~ND~START~
#
# # T1_PreFreeSurferPipelineWithoutT2.sh
#
# ## Author(s)
# 
# * Renaud Lopes
...
#
# 1. To average any image repeats (i.e. multiple T1w available)
...
# ### Image Files
#
# At least one T1 weighted image is required
...
Usage: T1_PreFreeSurferPipelineWithoutT2.sh [options]
...
# ------------------------------------------------------------------------------
log_SetToolName "T1_PreFreeSurferPipelineWithoutT2.sh"
...
# ------------------------------------------------------------------------------
#  Loop over the processing for T1w (just with different names). 
...
for TXw in ${Modalities} ; do
    ...
    # ACPC align T1w image to MNI Template to create native volume space
...
case $AvgrdcSTRING in 
    ...
    *) 
        ...
        ${RUN} ${FSLDIR}/bin/imcp ${T1wFolder}/${T1wImage}_acpc ${T1wFolder}/${T1wImage}_acpc_dc
        ${RUN} ${FSLDIR}/bin/imcp ${T1wFolder}/${T1wImage}_acpc_brain ${T1wFolder}/${T1wImage}_acpc_dc_brain
...
${RUN} ${FSLDIR}/bin/imcp ${T1wFolder}/${T1wImage}_acpc_dc ${T1wFolder}/${T1wImage}_acpc_dc_restore
${RUN} ${FSLDIR}/bin/imcp ${T1wFolder}/${T1wImage}_acpc_dc_brain ${T1wFolder}/${T1wImage}_acpc_dc_restore_brain
...
${RUN} T1_AtlasRegistrationToMNI152_FLIRTandFNIRT.sh \
