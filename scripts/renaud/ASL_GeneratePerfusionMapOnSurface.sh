...
then
	echo ""
	echo "Usage: ASL_GeneratePerfusionMapOnSurface.sh -sd <SUBJECTS_DIR>  -subj <SUBJ_ID>  -i <input>  -o <path>  [-fwhmsurf <value>  -fwhmvol <value>  -rmframe <value>  -sub <value>  -te <value> -pld <value>  -dur <value>  -doPve  -doCor  -pvenew "
	echo ""
...
# pre-smoothing for surface projection
if [ ${prefwhmvol} -gt 0 ]
then
/usr/local/matlab11/bin/matlab -nodisplay <<EOF
	...
	matlabbatch{end+1}.spm.spatial.smooth.data = cellstr(spm_select('ExtFPList',tempa,['^',tempb,tempc],1:1e4));
	matlabbatch{end}.spm.spatial.smooth.fwhm   = [${prefwhmvol} ${prefwhmvol} ${prefwhmvol}];
	matlabbatch{end}.spm.spatial.smooth.dtype  = 0;
	matlabbatch{end}.spm.spatial.smooth.im     = 0;
	matlabbatch{end}.spm.spatial.smooth.prefix = ['s' num2str(${prefwhmvol})];
	
	spm_jobman('run',matlabbatch);
	
EOF
else
     cp ${DIR}/${asldir}/asl.rem.mc.nii ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.nii
fi
ASL=${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.nii
...
# Project on native surface
# mask 
# lh
mri_vol2surf --mov ${DIR}/${asldir}/asl_mask_dil.nii --reg ${DIR}/${asldir}/asl.reg --trgsubject ${SUBJ} --interp nearest --projfrac 0.5 --hemi lh --o ${DIR}/${asldir}/brain.lh.nii --noreshape --cortex --surfreg sphere.reg
mri_binarize --i ${DIR}/${asldir}/brain.lh.nii --min .00001 --o ${DIR}/${asldir}/brain.lh.nii
# rh
mri_vol2surf --mov ${DIR}/${asldir}/asl_mask_dil.nii --reg ${DIR}/${asldir}/asl.reg --trgsubject ${SUBJ} --interp nearest --projfrac 0.5 --hemi rh --o ${DIR}/${asldir}/brain.rh.nii --noreshape --cortex --surfreg sphere.reg
mri_binarize --i ${DIR}/${asldir}/brain.rh.nii --min .00001 --o ${DIR}/${asldir}/brain.rh.nii

# asl
# lh
mri_vol2surf --mov ${ASL} --reg ${DIR}/${asldir}/asl.reg --trgsubject ${SUBJ} --interp trilin --projfrac 0.5 --hemi lh --o ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.lh.nii --noreshape --cortex --surfreg sphere.reg
mris_fwhm --s ${SUBJ} --hemi lh --smooth-only --i ${DIR}/${asldir}/s1.5asl.rem.mc.lh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.lh.nii --mask ${DIR}/${asldir}/brain.lh.nii
# rh
mri_vol2surf --mov ${ASL} --reg ${DIR}/${asldir}/asl.reg --trgsubject ${SUBJ} --interp trilin --projfrac 0.5 --hemi rh --o ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.rh.nii --noreshape --cortex --surfreg sphere.reg
mris_fwhm --s ${SUBJ} --hemi rh --smooth-only --i ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.rh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.rh.nii --mask ${DIR}/${asldir}/brain.rh.nii
...
/usr/local/matlab11/bin/matlab -nodisplay <<EOF
  ...
  % surface
  aslFile{1}='${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.lh.nii';
  aslFile{2}='${DIR}/${asldir}/s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.rh.nii';
  Compute_PerfAndCBFMap_OnSurface(aslFile,${PLD},${LabelDur},${TE},${subMethod},FirstimageType);
  
  % volume
  Compute_PerfAndCBFMap_Bis('${DIR}/${asldir}/s${fwhmvol}asl.rem.mc.nii','${DIR}/${asldir}/asl_mask_dil.nii',${PLD},${LabelDur},${TE},${subMethod},FirstimageType);
...
# CBF and PERF maps computed on surface
# fsaverage

# perfusion map
echo "mri_surf2surf --srcsubject ${SUBJ} --srchemi lh --srcsurfreg sphere.reg --trgsubject fsaverage --trghemi lh --trgsurfreg sphere.reg --sval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.lh.nii --sfmt curv --noreshape --no-cortex --tval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --tfmt curv"
mri_surf2surf --srcsubject ${SUBJ} --srchemi lh --srcsurfreg sphere.reg --trgsubject fsaverage --trghemi lh --trgsurfreg sphere.reg --sval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.lh.nii --sfmt curv --noreshape --no-cortex --tval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --tfmt curv
echo "mris_fwhm --s fsaverage --hemi lh --smooth-only --i ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${fwhmsurf}meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --mask ${DIR}/${outdir}/masks/brain.fsaverage.lh.nii"
mris_fwhm --s fsaverage --hemi lh --smooth-only --i ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${fwhmsurf}meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --mask ${DIR}/${outdir}/masks/brain.fsaverage.lh.nii
mri_surf2surf --srcsubject ${SUBJ} --srchemi rh --srcsurfreg sphere.reg --trgsubject fsaverage --trghemi rh --trgsurfreg sphere.reg --sval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.rh.nii --sfmt curv --noreshape --no-cortex --tval ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --tfmt curv
mris_fwhm --s fsaverage --hemi rh --smooth-only --i ${DIR}/${asldir}/meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${fwhmsurf}meanPERF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --mask ${DIR}/${outdir}/masks/brain.fsaverage.rh.nii

# CBF map
mri_surf2surf --srcsubject ${SUBJ} --srchemi lh --srcsurfreg sphere.reg --trgsubject fsaverage --trghemi lh --trgsurfreg sphere.reg --sval ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.lh.nii --sfmt curv --noreshape --no-cortex --tval ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --tfmt curv
mris_fwhm --s fsaverage --hemi lh --smooth-only --i ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${fwhmsurf}meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.lh.nii --mask ${DIR}/${outdir}/masks/brain.fsaverage.lh.nii
mri_surf2surf --srcsubject ${SUBJ} --srchemi rh --srcsurfreg sphere.reg --trgsubject fsaverage --trghemi rh --trgsurfreg sphere.reg --sval ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.rh.nii --sfmt curv --noreshape --no-cortex --tval ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --tfmt curv
mris_fwhm --s fsaverage --hemi rh --smooth-only --i ${DIR}/${asldir}/meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --fwhm ${fwhmsurf} --o ${DIR}/${asldir}/s${fwhmsurf}meanCBF_${subMethod}_s${prefwhmvol}asl.rem.mc.s${fwhmsurf}.fsaverage.rh.nii --mask ${DIR}/${outdir}/masks/brain.fsaverage.rh.nii
...
/usr/local/matlab11/bin/matlab -nodisplay <<EOF
      ...
      matlabbatch{end+1}.spm.spatial.normalise.est.subj.vol        = cellstr('${DIR}/${asldir}/T1.nii');
      ...
      clear matlabbatch 
      matlabbatch = {};	
      temp{1}='${DIR}/${asldir}/rmeanPERF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.nii';
      temp{2}='${DIR}/${asldir}/rmeanCBF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.nii';
      if ${DoPVE}==1
	  temp{3}='${DIR}/${asldir}/rmeanPERF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.pve.nii';
	  temp{4}='${DIR}/${asldir}/rmeanCBF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.pve.nii';
      end
      matlabbatch{end+1}.spm.spatial.normalise.write.subj.def      = cellstr('${DIR}/${asldir}/y_T1.nii');
      matlabbatch{end}.spm.spatial.normalise.write.subj.resample   = temp;
...
/usr/local/matlab11/bin/matlab -nodisplay <<EOF
	...
	spm_jobman('initcfg');
	matlabbatch = {};
	temp{1}='${DIR}/${asldir}/wrmeanPERF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.nii';
	temp{2}='${DIR}/${asldir}/wrmeanCBF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.nii';
	if ${DoPVE}==1
	    temp{3}='${DIR}/${asldir}/wrmeanPERF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.pve.nii';
	    temp{4}='${DIR}/${asldir}/wrmeanCBF_${subMethod}_s${fwhmvol}asl.rem.mc.anat.pve.nii';
	end

	matlabbatch{end+1}.spm.spatial.smooth.data = temp;
	matlabbatch{end}.spm.spatial.smooth.fwhm   = [${fwhmvol} ${fwhmvol} ${fwhmvol}];
	matlabbatch{end}.spm.spatial.smooth.dtype  = 0;
	matlabbatch{end}.spm.spatial.smooth.im     = 0;
	matlabbatch{end}.spm.spatial.smooth.prefix = ['s' num2str(${fwhmvol})];
