...
echo "Extracting subject : ${subject}"
	...
	cd /home/notorious/NAS/julien/Neurosclerodermie/Neurosclerodermie/ROUEN/data/${subject}/spectro
	

echo "      |_ Extracting CSO D"
csod_gpc=$(cat CSO_D/table | grep "GPC+PCh" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ GPC+PCh : $csod_gpc"
csod_naa=$(cat CSO_D/table | grep "NAA+NAAG" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ NAA+NAAG : $csod_naa"		
csod_glu=$(cat CSO_D/table | grep "Glu+Gln" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ Glu+Gln : $csod_glu"
csod_ins=$(cat CSO_D/table | grep "Ins" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ Ins : $csod_ins"

echo "      |_ Extracting CSO G"
csog_gpc=$(cat CSO_G/table | grep "GPC+PCh" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ GPC+PCh : $csog_gpc"
csog_naa=$(cat CSO_G/table | grep "NAA+NAAG" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ NAA+NAAG : $csog_naa"		
csog_glu=$(cat CSO_G/table | grep "Glu+Gln" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ Glu+Gln : $csog_glu"
csog_ins=$(cat CSO_G/table | grep "Ins" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ Ins : $csog_ins"


echo "      |_ Extracting NG D"
ngd_gpc=$(cat NG_D/table | grep "GPC+PCh" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ GPC+PCh : $ngd_gpc"
ngd_naa=$(cat NG_D/table | grep "NAA+NAAG" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ NAA+NAAG : $ngd_naa"	

echo "      |_ Extracting NG G"
ngg_gpc=$(cat NG_G/table | grep "GPC+PCh" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ GPC+PCh : $ngg_gpc"
ngg_naa=$(cat NG_G/table | grep "NAA+NAAG" | sed "s/\ \ */\ /g" | cut -d ' ' -f 4 )
echo "      |       |_ NAA+NAAG : $ngg_naa"
