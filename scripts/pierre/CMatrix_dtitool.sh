...
then
	echo ""
	echo "Usage: CMatrix_dtitool.sh  -fs  <SubjDir>  -subj  <SubjName>  -parcname  <parcellation.annot>  -dti  <dti_eddy_corr>  -bvecs <bvecs>  -bvals <bvals>  -outdir  <outputDirectory>"
	echo ""
...
echo " "
echo "---------------------------------------------------------"
echo "Launch dti tool tractography"
fslsplit ${dti} ${outdir}/vol_tmp -t
gunzip ${outdir}/vol_tmp*nii.gz


matlab -nodisplay <<EOF
cd ${outdir}
bvecs_to_dtitool('${bvecs}', '${outdir}/bvecs_dtitool.txt');
create_mrstruct('${outdir}/vol_tmp*nii', '${outdir}/mrStruct');
mrstruct_add_info('${outdir}/mrStruct.mat', '${outdir}/bvecs_dtitool.txt', '${bvals}');
mrstruct_calculate_tensors('${outdir}/mrStruct.mat');
EOF

matlab -nodisplay <<EOF
cd ${outdir}
mrstruct_track('${outdir}/mrStruct_DTD.mat', 0.2);
EOF

touch ${outdir}/tractography.touch
fi

rm -f ${outdir}/vol_tmp*nii
...
echo "Connectome = getSurfaceConnectMatrix(${outdir}/lh.white_nlin.obj, ${outdir}/rh.white_nlin.obj, ${DIR}/label/lh.${parcname}, ${DIR}/label/rh.${parcname}, ${outdir}/lh.${parcname%.annot}.stats, ${outdir}/rh.${parcname%.annot}.stats, ${outdir}/mrStruct_FTR.mat);"
matlab -nodisplay <<EOF
cd ${HOME}
p = pathdef;
addpath(p);
cd ${outdir}
Connectome = getSurfaceConnectMatrix('${outdir}/lh.white_nlin.obj', '${outdir}/rh.white_nlin.obj', '${DIR}/label/lh.${parcname}', '${DIR}/label/rh.${parcname}', '${outdir}/lh.${parcname%.annot}.stats', '${outdir}/rh.${parcname%.annot}.stats', '${outdir}/mrStruct_FTR.mat');
