...
`basename $0` performs T1 anatomical brain processing where the following steps are currently applied:
  ...
  3. (Optional) registration to a template
...
echo "PearsonCorrelation,BVOL,GVol,WVol" >   ${OUTPUT_PREFIX}brainvols.csv
echo "${ccmetric},${bvol},${gvol},${wvol}" >>  ${OUTPUT_PREFIX}brainvols.csv
...
#
# Create QA/QC output:
#   - tiled mosaic of ${OUTPUT_PREFIX}BrainSegmentation0N4.nii.gz
################################################################################
