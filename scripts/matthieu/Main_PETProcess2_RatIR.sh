...
then
	echo ""
	echo "Usage: Main_PETProcess2_RatIR.sh -id <inputdir>  -od <outputdir> -t <MriTemplate> [options]"
	echo ""
		...
		do   
			...
			then 
				...
				do
					...
					then
# 						
						qbatch -N PET2_${subject}_$tp -q fs_q -oe ~/Logdir  PETProcess2_RatIR.sh -id ${INPUT_DIR}/${subject}/${tp} -od ${OUTPUT_DIR}/${subject}/${tp} -t ${TEMP_PATH}
						sleep 5
					else
