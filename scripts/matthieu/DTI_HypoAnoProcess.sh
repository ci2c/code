...
then
	echo ""
	echo "Usage: DTI_HypoAnoProcess.sh -id <InputDir> -subjid <SubjId> -fs <SubjDir> -od <OutputDir> -f <ListeROIsPath>"
	echo ""
...
# 		matlabbatch{end}.spm.spatial.coreg.estwrite.source = {'${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/t1_${base}_las.nii,1'};
# 		matlabbatch{end}.spm.spatial.coreg.estwrite.other = {
# 								      '${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/wm_mask_${base}.nii,1'
# 								      };
...
# 	DTI_Tracto_WB.sh ${base} ${SUBJ_ID} ${OUTPUT_DIR} ${lmax} ${Nfiber}
# 	sleep 1
# done
# 
# # ################################
...
# 			
# 		# SPM coregister estimation
# 		# Then reslice ROI to DTI space
# 		matlab -nodisplay <<EOF
# 		spm('defaults', 'FMRI');
# 		spm_jobman('initcfg');
# 		matlabbatch={};
# 		
# 		% Open the text file containinfg paths of the ROIs nifti files
# 		fid = fopen('${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/PathsROIs', 'r');
...
# 		end
# 				
# 		matlabbatch{end+1}.spm.spatial.coreg.estimate.ref = {'${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/b0.nii,1'};
# 		matlabbatch{end}.spm.spatial.coreg.estimate.source = {'${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/t1_${base}_las.nii,1'};
# 		matlabbatch{end}.spm.spatial.coreg.estimate.other = CellF;
...
# 				
# 		matlabbatch{end+1}.spm.spatial.coreg.write.ref = {'${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/b0.nii,1'};
# 		matlabbatch{end}.spm.spatial.coreg.write.source = CellF;
...
# 		while read Roi
# 		do
# 			gunzip -f ${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/r${Roi}b_${base}_LAS.nii.gz
# 		done < ${OUTPUT_DIR}/${SUBJ_ID}/Connectum_${base}/ROIs.txt
# 		
# # 		qbatch -N ${SUBJ_ID}_${base}_DTI_NbF -q M32_q -oe ~/Logdir 
...
##############################################################
## Step 13. Coregister FS aparc_aseg2009 + Hypothalamus masks
##############################################################

if [ -s ${FILE_PATH}/ListeROIsConnectum.txt ]
then
	for dti in $(ls ${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/dti[1-9].nii.gz)
	do
		base=`basename ${dti}`
		...
		if [ ! -d ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base} ]
		then
			mkdir ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}
		else
			rm -rf ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/*
		fi
	   
		mri_convert ${FS_DIR}/${SUBJ_ID}/mri/aparc.a2009s+aseg.mgz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/aparc.a2009s+aseg_LAS.nii.gz --out_orientation LAS
		...
		do
			if [ -s ${FS_DIR}/${SUBJ_ID}/${base}/${Roi}.nii ]
			then
				mri_convert ${FS_DIR}/${SUBJ_ID}/${base}/${Roi}.nii ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/${Roi}_LAS.nii --out_orientation LAS	
	
				mri_binarize --i ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/${Roi}_LAS.nii --min 0.1 --binval ${index} --o ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/${Roi}_v${index}_LAS.nii.gz
			fi
			index=$[$index+1]
		done < ${FILE_PATH}/ListeROIsConnectum.txt
		
		fslmaths ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/HA_RH_v1_LAS.nii.gz \
		-add ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/LHAa_RH_v2_LAS.nii.gz \
		-add ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/LHAt_RH_v3_LAS.nii.gz \
		${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/Hypo_Ano_LAS.nii.gz
		
		# 	-add ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/LHAa_LH_v2_LAS.nii.gz \

		cp -f ${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/t1_native_las.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/t1_${base}_las.nii.gz
		cp -f ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/aparc.a2009s+aseg_LAS.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/aparc.a2009s+aseg_${base}_LAS.nii.gz
		cp -f ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/Hypo_Ano_LAS.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/Hypo_Ano_${base}_LAS.nii.gz
			
		fslroi ${final_dti} ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/b0 0 1
			
		gunzip -f ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/b0.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/t1_${base}_las.nii.gz \
		${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/Hypo_Ano_${base}_LAS.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/aparc.a2009s+aseg_${base}_LAS.nii.gz
		...
		matlabbatch{end+1}.spm.spatial.coreg.estimate.ref = {'${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/b0.nii,1'};
		matlabbatch{end}.spm.spatial.coreg.estimate.source = {'${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/t1_${base}_las.nii,1'};
		matlabbatch{end}.spm.spatial.coreg.estimate.other = {
								      '${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/aparc.a2009s+aseg_${base}_LAS.nii,1'
								      '${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/Hypo_Ano_${base}_LAS.nii,1'								      
								      };
		matlabbatch{end}.spm.spatial.coreg.estimate.eoptions.cost_fun = 'nmi';
		matlabbatch{end}.spm.spatial.coreg.estimate.eoptions.sep = [4 2];
		matlabbatch{end}.spm.spatial.coreg.estimate.eoptions.tol = [0.02 0.02 0.02 0.001 0.001 0.001 0.01 0.01 0.01 0.001 0.001 0.001];
		...
		matlabbatch{end+1}.spm.spatial.coreg.write.ref = {'${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/b0.nii,1'};
...
EOF
		...
		gzip ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/*.nii
		
		# Remove NaNs
		fslmaths ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/raparc.a2009s+aseg_${base}_LAS.nii.gz -nan ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/raparc.a2009s+aseg_${base}_LAS.nii.gz
		fslmaths ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/rHypo_Ano_${base}_LAS.nii.gz -nan ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/rHypo_Ano_${base}_LAS.nii.gz
			
	done
fi

# ####################################################################################################################
# ## Step 14. Get Nb fibers for each hypothalamic ROI and intersection between hypothalamic ROI & FS segmentation ROI
# ####################################################################################################################

if [ -s ${FILE_PATH}/Hypo_LOI.txt -a -s ${FILE_PATH}/aparc2009LOI_HypoAno.txt ]
then
	for dti in $(ls ${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/dti[1-9].nii.gz)
# 	for dti in $(ls ${OUTPUT_DIR}/${SUBJ_ID}/dti_tracto/dti1.nii.gz)
	do
		base=`basename ${dti}`
		base=${base%.nii.gz}
		
		gunzip -f ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/raparc.a2009s+aseg_${base}_LAS.nii.gz ${OUTPUT_DIR}/${SUBJ_ID}/ConnCortSubcort_${base}/rHypo_Ano_${base}_LAS.nii.gz
		
# 		qbatch -N ${SUBJ_ID}_${base}_DTI_hypothVSfsseg -q M32_q -oe /NAS/tupac/matthieu/Logdir 
		DTI_HypoAno_hypothVSfsseg.sh ${SUBJ_ID} ${base} ${OUTPUT_DIR} ${lmax} ${Nfiber}
		sleep 1
	done
fi