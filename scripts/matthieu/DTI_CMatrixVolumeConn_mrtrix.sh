...
then
	echo ""
	echo "Usage: DTI_CMatrixVolumeConn_mrtrix.sh  -fs  <SubjDir>  -subj  <SubjName>  -parcname  <parcellation.mgz>  -labels <labels_list.txt>  -dti  <dti_eddy_corr>  -bvecs <bvecs>  -bvals <bvals>  -outdir  <outputDirectory>  [-lmax <lmax>  -vs <size_x> <size_y> <size_z>  -N <Nfiber> -no_CM]"
	echo ""
...
nii2mnc ${outdir}/labels.nii ${outdir}/labels.mnc
  ...
  rm -rf ${outdir}/loi_temp
  mkdir ${outdir}/loi_temp

  loidir=${outdir}/loi_temp
  
#   cp -f ${outdir}/labels.mnc /tmp/labels_${subj}.mnc
  ...
  do
	...
	echo "mincmath ${outdir}/labels.mnc -segment -const2 ${inf} ${sup} ${loidir}/loi_${LOI_ID}.mnc -clobber"
	mincmath ${outdir}/labels.mnc -segment -const2 ${inf} ${sup} ${loidir}/loi_${LOI_ID}.mnc -clobber
...
echo " "
echo "---------------------------------"
echo "Connectome = getVolumeFibersROIs('${outdir}/labels_dti.nii', '${outdir}', '${LOI}', 30);"
matlab -nodisplay <<EOF
cd ${HOME}
p = pathdef;
addpath(p);
cd ${outdir}
Connectome = getVolumeFibersROIs('${outdir}/labels_dti.nii', '${outdir}', '${LOI}', 30);
Connectome.fibers = [];
...
if [ ! -f ${outdir}/fa.nii ] && [ ! -f ${outdir}/MD.nii ]
then
	mrconvert ${outdir}/fa.mif ${outdir}/fa.nii
	tensor2ADC ${outdir}/dt.mif - | mrmult - ${outdir}/rwm_mask_dti.mif ${outdir}/MD.mif
	mrconvert ${outdir}/MD.mif ${outdir}/MD.nii
fi

echo "Construct connectivity matrix"
echo "Connectome = computeDTIConnectMatrix('${outdir}', '${outdir}/labels_dti.nii', '${outdir}/fa.nii', '${outdir}/MD.nii', Connectome, 2, 30);"
matlab -nodisplay <<EOF
cd ${HOME}
p = pathdef;
addpath(p);
cd ${outdir}
load Connectome_${subj}
Connectome = computeDTIConnectMatrix('${outdir}', '${outdir}/labels_dti.nii', '${outdir}/fa.nii', '${outdir}/MD.nii', Connectome, 2, 30);
save Connectome_${subj} Connectome -v7.3
EOF
fi
