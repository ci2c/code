...
then
	echo ""
	echo "Usage: Main_PETProcess_RatMorv.sh -id <inputdir> -od <outputdir> -sm <smallmaskpath> -lm <largemaskpath> [options]"
	echo ""
		...
		do   
			...
			then 
				...
				do
					...
					then
# 						
						qbatch -N PET1_$tp -q fs_q -oe ~/Logdir PETProcess_RatMorv.sh -id ${INPUT_DIR}/${subject}/${tp} -od ${OUTPUT_DIR}/${subject}/${tp} -sm ${SM_PATH} -lm ${LM_PATH}
						sleep 2
					else
