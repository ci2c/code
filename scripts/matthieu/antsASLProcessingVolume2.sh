...
`basename $0` performs ASL processing based on ANTs tools.  Preprocessing of T1 images using antsCorticalThickness.sh is assumed.  The following steps are performed: 
  ...
  6) Warping all types CBF (CBF, PVC_CBF) to template space for VBM analysis.
  7) Compute PVC_CBF and registration of all types CBF (CBF, PVC_CBF) on T1 image
...
# # cbf="${OUTNAME}_meancbf.nii.gz"
# # obase="${outdir}${dirname}${outname}${outid}"
# # ref="${OUTNAME}_ref.nii.gz"
# # meanpcasl="${OUTNAME}_meanpcasl.nii.gz"
# # 
# # anat="${OUTPUT_DIR}/${SUBJ_ID}/CorticalThickness/CTBrainExtractionBrain.nii.gz"
# # mask="${OUTPUT_DIR}/${SUBJ_ID}/CorticalThickness/CTBrainExtractionMask.nii.gz"
# # 
# # # Create anatomical reference to align to      
# # ${ANTSPATH}/ImageMath 3 $ref m $anat $mask
# # ${ANTSPATH}/ResampleImageBySpacing 3 $ref $ref 3.0 3.0 3.0
# # 
# # # Motion correct    
# # ${ANTSPATH}/antsMotionCorr -d 3 -a $PCASL -o $meanpcasl
# #     
# # ${ANTSPATH}/antsMotionCorr -d 3 -o [ ASL_, ${PCASL}_mc, ${meanpcasl}_mc ] -u 1 -m MI[ $meanpcasl, $PCASL, 1, 32, Regular, 0.1 ] -t Affine[0.2] -i 25 -e 1 -f 1 -s 0 -l 0
# # 
# # ${ANTSPATH}/cbf_pcasl_robust_batch.R $PCASL $cbf
# # 
# # ${ANTSPATH}/antsIntermodalityIntrasubject.sh -d 3 -t 2 -a $cbf -i $meanpcasl -r $ref -R $anat -x $mask $warpflag -o ${outdir}${dirname}${outname}${outid} -T $template
...
# 
# # Split ASL 4D file
# echo "fslsplit $PCASL ${WD}/split/pcasl_ -t"
# fslsplit $PCASL ${WD}/split/pcasl_ -t
# 
# # Exclude first 4 frames
# echo "Exclude first 4 frames"
# for ((ind = 0; ind < ${remframe}; ind += 1))
# do
# 	filename=`ls -1 ${WD}/split/pcasl_* | sed -ne "1p"`
# 	rm -f ${filename}
# done
# 
# echo "fslmerge -t ${WD}/pcasl.rem.nii.gz ${WD}/split/*"
# fslmerge -t ${WD}/pcasl.rem.nii.gz ${WD}/split/*
...
# echo "gunzip ${OUTNAME}_mask.nii.gz"
# gunzip ${OUTNAME}_mask.nii.gz
# echo "mri_morphology ${OUTNAME}_mask.nii dilate 1 ${OUTNAME}_mask_dil.nii" 
# mri_morphology ${OUTNAME}_mask.nii dilate 1 ${OUTNAME}_mask_dil.nii
# gzip ${OUTNAME}_mask.nii.gz ${OUTNAME}_mask_dil.nii

logCmd ${ANTSPATH}ImageMath 3 ${OUTNAME}pCASLBrain.nii.gz m ${OUTNAME}AveragePCASL.nii.gz ${OUTNAME}_mask.nii.gz
...
# gunzip ${OUTNAME}brainmask.nii.gz
# mri_morphology ${OUTNAME}brainmask.nii dilate 1 ${OUTNAME}brainmask_dil.nii
# gzip ${OUTNAME}brainmask.nii ${OUTNAME}brainmask_dil.nii
...
# if [ $DoMC -eq 1 ]
# then
# 	mc-afni2 --i ${WD}/pcasl.rem.nii --t ${OUTNAME}AveragePCASL.nii --o ${WD}/pcasl.rem.mc.nii --mcdat ${WD}/pcasl.rem.mcdat
# 
#  	# Making external regressor from mc params
# 	mcdat2mcextreg --i ${WD}/pcasl.rem.mcdat --o ${WD}/mcprextreg
# fi
# 
...
#   
#   FirstimageType = 0;
#   Compute_PerfAndCBFMap_Bis('${WD}/pcasl.rem.mc.nii','${OUTNAME}_mask.nii',${PLD},${LabelDur},${TE},${subMethod},FirstimageType);
# EOF
# 
# gzip ${WD}/*.nii
