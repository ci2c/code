...
then
	echo ""
	echo "Usage: getGPUSurfaceConnectome.sh  -fs  <SubjDir>  -subj  <SubjName>  [-surf_lh surface_lh_path  -surf_rh surface_rh_path  -fib fibre_filename  -out outfile_name  -log log_dir]"
	echo ""
	...
	echo ""
	echo "Usage: getGPUSurfaceConnectome.sh  -fs  <SubjDir>  -subj  <SubjName>  [-surf_lh surface_lh_path  -surf_rh surface_rh_path  -fib fibre_filename  -out outfile_name  -log logdir]"
	exit 1
fi
...
then
	...
	do
		...
		then
			echo "qbatch -N gpuSplit_${i} -oe ${log_dir} -q  gpu_one_job_q getGPUSurfaceConnectome.sh -fs ${fs} -subj ${subj} -fib ${split_name} -out ${out_split_name} -surf_lh ${surf_lh} -surf_rh ${surf_rh}"
			TEMP=`qbatch -N gpuSplit_${i} -oe ${log_dir} -q gpu_one_job_q@qotsa getGPUSurfaceConnectome.sh -fs ${fs} -subj ${subj} -fib ${split_name} -out ${out_split_name} -surf_lh ${surf_lh} -surf_rh ${surf_rh}`
	...
	qbatch ${JOB_ID} -N GPU_postproc -oe ${log_dir} -q fs_q PostProcessGPUSurfaceConnectome.sh -fs ${fs} -subj ${subj} -mat_name ${out_name%.mat}
	
else
	# No corresponding split found, launch job locally
matlab -nodisplay <<EOF
Connectome = getFastTriangleConnectMatByGPU('${surf_lh}', '${surf_rh}', '${fibers}', '${ref_vol}', '${dti_vol}');
