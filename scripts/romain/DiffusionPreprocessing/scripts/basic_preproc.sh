...
nPEsteps=$(($dimP/2 - 1))                         #If GRAPPA is used this needs to include the GRAPPA factor!
#RV j'ai divisé par 2 pour tenir compte du sense facteur (eq. GRAPPA cf commentaire ci dessus)
#0018,9069 [FD] Parallel Reduction Factor In-plane: 2.0
...
if [ -f ${rawdir}/${baseNeg}_SeriesCorrespVolNum.txt ]
then
	tmp_indx=1
	while read line ; do  #Read SeriesCorrespVolNum.txt file
	...
	echo "0 Negative volumes was provided"
	################################################################################################
	## Move files to appropriate directories 
	################################################################################################
	echo "Move files to appropriate directories"
